...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
	<script type="text/javascript">
		...
		function init() {
			...
			grid.cellClickEvent(function(rowIdx, colIdx) {
				...
				if(colIdx == "USER_NM") {
		        	var param = {
		            		custCd : grid.getCellValue(rowIdx,'BUYER_CD') ,
							callBackFunction: "selectUserIdPopupIntoGrid"
						};
			...
			grid.addRowEvent(function() {
				...
				addParam = [{
					"BUYER_NM": EVF.V('BUYER_NM'),
					"BUYER_CD_ORI": EVF.V('BUYER_CD'),
		...
		function doSearch() {
			...
			store.setGrid([grid]);
			store.load(baseUrl + 'selectTaskPersonInCharge.so', function() {
		...
		function doSave() {
			...
			EVF.confirm("${msg.M0021 }", function() {
				...
				store.load(baseUrl + 'saveTaskPersonInCharge.so', function(){
		...
		function doDelete() {
			...
			EVF.confirm("${msg.M0013 }", function() {
				...
				store.setGrid([grid]);
				store.getGridData(grid, 'sel');
				store.load(baseUrl + 'deleteTaskPersonInCharge.so', function(){
					EVF.alert(this.getResponseMessage(), function() {
						doSearch();
					});
				});
		...
		function selectUserIdPopupIntoGrid(data) {
			...
			var selrowIdxs = grid.getSelRowValue();
			if (selectedRow.length > 1) {
				EVF.confirm('${MAUA0020_001}', function() {
	...
	<e:window id="MAUA0020" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
		<e:searchPanel id="form" title="${form_CAPTION_N }" columnCount="3" labelWidth="${labelWidth }" useTitleBar="false" onEnter="doSearch">
			<e:row>
				...
				<e:field>
					<e:search id="BUYER_CD" name="BUYER_CD" value="" width="40%" maxLength="${form_BUYER_CD_M}" onIconClick="getBuyerCd" disabled="${form_BUYER_CD_D}" readOnly="${form_BUYER_CD_RO}" required="${form_BUYER_CD_R}" maskType="${form_BUYER_CD_MT}" />
			...
			<e:row>
				...
				<e:field>
					<e:search id="DEPT_CD" name="DEPT_CD" value="${data.DEPT_CD}" width="40%" maxLength="${form_DEPT_CD_M}" onIconClick="${form_DEPT_CD_D ? 'everCommon.blank' : 'onIconClickDEPT_CD'}" disabled="${form_DEPT_CD_D}" readOnly="${form_DEPT_CD_RO}" required="${form_DEPT_CD_R}"  maskType="${form_DEPT_CD_MT}" />
					<e:inputText id="DEPT_NM" name="DEPT_NM" value="${data.DEPT_NM}" width="60%" maxLength="${form_DEPT_NM_M}" disabled="${form_DEPT_NM_D}" readOnly="${form_DEPT_NM_RO}" required="${form_PO_DEPT_NM_R}"  maskType="${form_DEPT_NM_MT}" />
				</e:field>
				<e:field colSpan="2">
				</e:field >
			</e:row>
