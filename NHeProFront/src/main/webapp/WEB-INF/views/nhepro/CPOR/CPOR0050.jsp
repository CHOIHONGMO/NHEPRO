...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    <script type="text/javascript">
        ...
        function doUpdateChange() {
            ...
            if(EVF.V("PIC_USER_ID") == "") { return EVF.alert("${CPOR0050_001}"); }
            ...
            for(var i in selRowValue) {
                ...
                if(rowData.PROGRESS_CD > "100") {
                    return EVF.alert("${CPOR0050_009}");
                }
				
             	// 2021.02.08 중앙회 요청 "관리자직무"를 갖는 사람은 담당자 변경 가능하도록 추가
                if(!changeFlag) {
	                if(rowData.INSPECT_USER_ID != "${ses.userId}") {
	                    return EVF.alert("${CPOR0050_018}");
	                }
                }
            }

            EVF.confirm("${CPOR0050_005}", function () {
                store.setGrid([grid]);
                store.getGridData(grid, "sel");
                store.load(baseUrl + "cpor0050_doUpdateChange.so", function() {
                    doSearch();
        ...
        function doSearch() {
            ...
            store.load(baseUrl + "cpor0050_doSearch.so", function() {
        ...
        function doApprovalPop() {
            if (grid.getSelRowCount() == 0) { return EVF.alert("${msg.M0004}"); }
            if (grid.getSelRowCount() > 1) { return EVF.alert("${CPOR0050_014}"); }

            var selRowValue = grid.getSelRowValue();
            for(var i in selRowValue) {
                var rowData = selRowValue[i];
                // 500(반려), 700(승인), E/P(결재중, 승인)
                if(rowData.PROGRESS_CD == "500" || rowData.PROGRESS_CD == "700" || rowData.SIGN_STATUS == "E" || rowData.SIGN_STATUS == "P") {
                    return EVF.alert("${CPOR0050_010}");
                }

                if(rowData.INSPECT_USER_ID != "${ses.userId}") {
                    return EVF.alert("${CPOR0050_002}");
                }

                if(rowData.EXEC_AUTH == '0') {
                    return  EVF.alert("${CPOR0050_015}")
                }
            }
    ...
    <e:window id="CPOR0050" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
        <e:searchPanel id="sp" title="${form_CAPTION_N}" labelWidth="${labelWidth}" useTitleBar="false" width="100%" columnCount="3" onEnter="doSearch">
            ...
            <e:row>
                ...
                <e:field>
                    <e:search id="INSPECT_USER_ID" name="INSPECT_USER_ID" value="${ses.userId}" width="40%" maxLength="${form_INSPECT_USER_ID_M}" onIconClick="${form_INSPECT_USER_ID_RO ? 'everCommon.blank' : 'onIconClickINSPECT_USER_ID'}" disabled="${form_INSPECT_USER_ID_D}" readOnly="${form_INSPECT_USER_ID_RO}" required="${form_INSPECT_USER_ID_R}" maskType="${form_INSPECT_USER_ID_MT}" placeHolder="개인번호" />
                    <e:inputText id="INSPECT_USER_NM" name="INSPECT_USER_NM" value="${ses.userNm}" width="60%" maxLength="${form_INSPECT_USER_NM_M}" disabled="${form_INSPECT_USER_NM_D}" readOnly="${form_INSPECT_USER_NM_RO}" required="${form_INSPECT_USER_NM_R}" style="${imeMode}" maskType="${form_INSPECT_USER_NM_MT}" placeHolder="성명"/>
                </e:field>
