...
import com.st_ones.nhepro.SETR.SETR0040_Mapper;
 ...
 ******************************************************************************
 * </pre>
 * @File Name : SETR0040_Service.java
 * @date 2018. 01. 30.
 * @version 1.0
 */

@Service(value = "SETR0040_Service")
public class SETR0040_Service extends BaseService {

    @Autowired MessageService msg;
    @Autowired SETR0040_Mapper setr0040_mapper;
     ...
     */
    public List<Map<String, Object>> setr0040_doSearch(Map<String, String> param) {
    	
        return setr0040_mapper.setr0040_doSearch(param);
    }

    // 만족도 저장
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    public String setr0040_doSave(List<Map<String, Object>> gridDatas) throws Exception {
    	
        for (Map<String, Object> gridData : gridDatas) {
            setr0040_mapper.setr0040_doSave(gridData);
        }
     ...
     */
    public Map<String, Object> setr0041_doSearch(Map<String, String> param) {
    	
        return setr0040_mapper.setr0041_doSearch(param);
    }

    // 요청
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    public String setr0041_doSaveReq(Map<String, String> param) throws Exception {
        ...
        if(!"".equals(param.get("VC_NO")) && param.get("VC_NO") != null) {
            setr0040_mapper.setr0041_doUpdate(param);
        }
        else {
            String COMPANY_CD = "";
            if("USNI".equals(VOC_OBJ)) {	// VOC대상: 운영사
                ...
                COMPANY_CD = param.get("VOC_OBJ_SUP_CD");
            }
            
            String VC_NO = docNumService.getDocNumber(COMPANY_CD, "VC");
            param.put("VC_NO", VC_NO);
            param.put("COMPANY_CD", COMPANY_CD);
            param.put("VIEW_TYPE", "VOC");
            setr0040_mapper.setr0041_doInsert(param);
        }
        ...
        if("USNI".equals(VOC_OBJ)) {	// VOC대상: 운영사
            ...
            sendInfo.put("CTRL_CD" , "BR100");
            sendInfo.put("BUYER_CD", param.get("VOC_OBJ_SUP_CD"));
            sendInfo.put("DEPT_CD" , param.get("VOC_OBJ_DEPT_CD"));
        }

        String linkUrl = PropertiesManager.getString("eversrm.urls.maintain.real") ;
        List<Map<String, String>> mapBACP = setr0040_mapper.setr0041_getSelectBACP(sendInfo);    // 직무에서 조회, 담당자 아이디, 이메일, 전화번호
        if(mapBACP != null){
            for(Map<String, String> data : mapBACP) {
            	String subject = "[전자구매시스템] 협력사 [" + param.get("REQ_COM_NM") + " - " + param.get("REQ_USER_NM") + "]가 [" + data.get("VOC_TYPE_NM") + "] 관련 VOC를 등록하였습니다";
            	
                //SMS
                if("1".equals(data.get("SMS_FLAG")) || "USNI".equals(VOC_OBJ)) {
                    Map<String,String> smsMap = new HashMap<String,String>();
                    smsMap.put("CONTENTS", subject);
                    smsMap.put("REF_MODULE_CD", "SVOC03");
                    smsMap.put("RECV_USER_ID", data.get("CTRL_USER_ID"));
                    
                    // 2021.07.13 : 협력사에서 VOC 등록시 고객사에 SMS 수수료 부과
                    if("1".equals(data.get("SMS_FLAG"))) {
                        ...
                        smsMap.put("USER_ID", data.get("USER_ID"));     	// 고객사 사용자 ID
                ...
                if("1".equals(data.get("MAIL_FLAG")) || "USNI".equals(VOC_OBJ)) {
                    ...
                    mailMap.put("REF_MODULE_CD", "MVOC03");
    ...
    // 저장
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    public Map<String, String> setr0041_doSave(Map<String, String> param) throws Exception {
        ...
        if(!"".equals(param.get("VC_NO")) && param.get("VC_NO") != null) {
            if("100".equals(PROGRESS_CD)) {
                ...
                setr0040_mapper.setr0041_doUpdate2(param);
            }
        }
        else {
            String COMPANY_CD = "";
        ...
        if("500".equals(PROGRESS_CD)) {
            ...
            Map<String,String> smsMap = new HashMap<String,String>();
            smsMap.put("CONTENTS", subject);
            smsMap.put("REF_MODULE_CD", "SVOC04");
            smsMap.put("RECV_USER_ID", param.get("REQ_USER_ID"));
            
            // 2021.07.13 : 협력사 VOC등록 완료시 요청고객사에 SMS수수료 부과
            Map<String, String> costInfo = setr0040_mapper.costSmsInfo(param);
            ...
            mailMap.put("CONTENTS", content);
            mailMap.put("REF_MODULE_CD", "MVOC04");
    ...
    // 삭제
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    public String setr0041_doDelete(Map<String, String> param) throws Exception {
    	
        setr0040_mapper.setr0041_doDelete(param);
