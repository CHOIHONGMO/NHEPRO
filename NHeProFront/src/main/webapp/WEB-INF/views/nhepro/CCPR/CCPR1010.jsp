...
<e:ui>
	<script type="text/javascript">
		...
		var grid = {};
		var baseUrl = "/nhepro/CCPR/CCPR1010";
		...
		function init() {
			...
			if(type == 'B'){
				...
				grid.hideCol('PDF_ATT_FILE_NUM', true);
			}
		...
		function doSend() {
            ...
            for(var i in selRowId) {
            	...
            	if (grid.getCellValue(selRowId[i], "SIGN_STATUS") != 'E' || grid.getCellValue(selRowId[i], "PROGRESS_CD") != 4210) {
            		return EVF.alert("${CCPR1010_005}"); // 계약대기인 건만 전자서명 요청할 수 있음
                }
        	...
        	//EVF.confirm('${CCPR1010_006}', function () {
				store.setGrid([grid]);
...
/*             	var progressCd = grid.getCellValue(selRowId[i], "PROGRESS_CD");
	...
	<e:window id="CCPR1010" onReady="init" initData="${initData}" title="${fullScreenName }" breadCrumbs="${breadCrumb }">
