...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    ...
    <script type="text/javascript">
        ...
        function init() {
            ...
            grid.cellClickEvent(function(rowIdx, colIdx, value) {
            	if (colIdx == "CTRL_NM") {
                    var param = {
                        ...
                        COMPANY_TYPE : "B",
                        callBackFunction : "setSingleCtrlInfo",
                        rowIdx : rowIdx
                    };
            ...
            grid.addRowEvent(function() {
            	var param = {
                        BUYER_CD : "${ses.manageCd}",
                        COMPANY_TYPE : "B",
                        callBackFunction : "setMultiCtrlInfo"
                    };
        ...
        function doIdcheck() {
            ...
            if(EVF.isEmpty(EVF.V("USER_ID"))){
                ...
                store.load(baseUrl + 'ccur0031_doCheckUserId.so', function() {
                    if(this.getParameter("POSSIBLE_FLAG") == "N") {
                        EVF.V("ID_CHECK", "");
                        return EVF.alert("${CCUR0031_006}");
                    } else {
                        EVF.V("ID_CHECK", "Y");
                        EVF.alert("${CCUR0031_001}");
                    }
                }, false);
            }
        }
        ...
        function doSave() {
            ...
            if(EVF.V("ID_CHECK") == ""){ return EVF.alert("${CCUR0031_002}"); }

            EVF.confirm(btnMsg, function () {
                store.load(baseUrl + 'ccur0031_doSave.so', function () {
                    EVF.alert(this.getResponseMessage(), function() {
                        ...
                        window.location.href = '/nhepro/CCUR/CCUR0031/view.so?' + $.param(param);
                    });
                });
            });
        }
        ...
        function doSaveAuth() {
            ...
            for(var i = 0; i < rowIds.length; i++) {
                for (var j = (i+1); j < rowIds.length; j++) {
                    if (grid.getCellValue(rowIds[j], 'CTRL_CD') == grid.getCellValue(rowIds[i], 'CTRL_CD')) {
                        ...
                        return EVF.alert(grid.getCellValue(rowIds[i], 'CTRL_NM') + "은(는) " + "${CCUR0031_024}");
                    }
                }
            }
            ...
            EVF.confirm("${msg.M0021 }", function () {
                ...
                store.load(baseUrl + 'ccur0031_doSaveAuth.so', function() {
                    EVF.alert(this.getResponseMessage(), function() {
                        doSearchAuth();
                    });
                });
        ...
        function doSearchAuth() {
            ...
            var store = new EVF.Store();
            store.setGrid([grid]);
            store.load(baseUrl + 'ccur0031_doSearchAuth.so', function() {
        ...
        // 권한 여러개 추가
        function setMultiCtrlInfo(data) {
        	for(var i in data) {
                var rowIdx = grid.addRow(data[i]);
				grid.setCellValue(rowIdx, 'CTRL_CD', data[i].CTRL_CD);
			    grid.setCellValue(rowIdx, 'CTRL_NM', data[i].CTRL_NM);
			    grid.setCellValue(rowIdx, 'USE_FLAG', '1');
            }
        }
        
        // 권한 1개씩 추가
        function setSingleCtrlInfo(data) {
        ...
        // 2021.05.14 사용자 ID 입력 시 금지어 체크 추가
        function IdcheckId(){
        	// 2021.05.12 message가 안뜨는 오류로 로직 변경
       ...
       //2021.05.14 이메일 입력시 금지어 체크 추가
        ...
        function parentDeptCdSearch() {
            ...
            if (EVF.isEmpty(EVF.V("COMPANY_CD"))) { return EVF.alert("${CCUR0031_0005}"); }
        ...
        function doChiefUserSearch() {
            ...
            if( EVF.isEmpty(custCd) ) { return EVF.alert("${CCUR0031_011}"); }
    ...
    <e:window id="CCUR0031" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">

        <e:inputHidden id="ORI_USER_ID" name="ORI_USER_ID" value="${formData.USER_ID }" />
        <e:inputHidden id="USER_TYPE" name="USER_TYPE" value="B" />
        <e:inputHidden id="COMPANY_CD" name="COMPANY_CD" value="${formData.COMPANY_CD }" />
        ...
        <e:searchPanel id="form" title="${form_CAPTION_N }" labelWidth="${labelWidth}" width="100%" columnCount="2" useTitleBar="false">
            ...
            <e:row>
                ...
                <e:field>
                    <e:inputText id="TEL_NUM" name="TEL_NUM" value="${formData.TEL_NUM}" placeHolder="${CCUR0031_INPUT_T2 }" width="${form_TEL_NUM_W}" maxLength="${form_TEL_NUM_M}" disabled="${form_TEL_NUM_D}" readOnly="${form_TEL_NUM_RO}" required="${form_TEL_NUM_R}" maskType="${form_TEL_NUM_MT}" onChange="checkTelNo" data="TEL_NUM" />
                </e:field>
            ...
            <e:row>
                ...
                <e:field>
                    <e:inputText id="FAX_NUM" name="FAX_NUM" placeHolder="${CCUR0031_INPUT_T2 }" value="${formData.FAX_NUM}" width="${form_FAX_NUM_W}" maxLength="${form_FAX_NUM_M}" disabled="${form_FAX_NUM_D}" readOnly="${form_FAX_NUM_RO}" required="${form_FAX_NUM_R}" maskType="${form_FAX_NUM_MT}" onChange="checkTelNo" data="FAX_NUM" />
                </e:field>
                <e:label for="CELL_NUM" title="${form_CELL_NUM_N}"></e:label>
                <e:field>
                    <e:inputText id="CELL_NUM" name="CELL_NUM"  placeHolder="${CCUR0031_INPUT_T2 }" value="${formData.CELL_NUM}" width="${form_CELL_NUM_W}" maxLength="${form_CELL_NUM_M}" disabled="${form_CELL_NUM_D}" readOnly="${form_CELL_NUM_RO}" required="${form_CELL_NUM_R}" maskType="${form_CELL_NUM_MT}" onChange="checkCellNo" data="CELL_NUM" />
                </e:field>
            </e:row>
            <e:row>
            ...
            <e:row>
                ...
                <e:field>
					...
					<e:inputHidden id="CHIEF_USER_ID" name="CHIEF_USER_ID"/>
                </e:field>
        ...
        <e:buttonBar id="buttonTop" align="right" width="100%" title="${CCUR0031_CAPTION1 }">
