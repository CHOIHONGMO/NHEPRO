...
import com.st_ones.batch.nhebatch.service.BNH0017_Service;
...
@Service
public class BNH0017 extends EverJob {
    @Autowired private BNH0001_Service bnh0001_service;
    @Autowired private BNH0017_Service bnh0017_service;
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (super.isAllowedToRunBatch(context)) {
            ...
            bnh0017_service = SpringContextUtil.getBean(BNH0017_Service.class);

            String msg = "";
            int totalCount = 0;

            try {
                msg = bnh0017_service.doExecService(new HashMap<String, String>());
                jobRlt = "S";
            } catch (Exception e) {
            	...
            	if( telNums != null ) {
	            	for (String telNum : telNums) {
	                    try {
	    	                ...
	    	                smsMap.put("CONTENTS", "[전자구매Batch] 12개월 미접속 사용자 정보이관(BNH0017) I/F시 오류가 발생");
                ...
                try {
                    ...
                    logData.put("JOB_NM", "12개월 미접속 사용자 정보이관 및 48개월 미접속 삭제");
                    logData.put("JOB_KEY", "");
