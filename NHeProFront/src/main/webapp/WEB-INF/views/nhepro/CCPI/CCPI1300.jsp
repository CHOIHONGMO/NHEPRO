...
<e:ui>
	...
	var baseUrl = "/nhepro/CCPI/CCPI1300";
	...
	function doSearchEmployee() {
		...
		store.setGrid([gridV]);
		store.load(baseUrl + "/ccpi1300_getSavedEmpListForBundleContract.so", function () {
	...
	function doSave() {
		...
		EVF.confirm("${msg.M0021 }", function () {
			...
			store.doFileUpload(function () {
				store.load(baseUrl + "/ccpi1300_doSave.so", function () {
	...
	function doCopy() {
		...
		//행복사 항목 : 근로시간 , 휴게시간
		EVF.confirm('${CCPI1300_009}', function() {
	...
	function doReqSign() {
		...
		store.doFileUpload(function () {
			store.load(baseUrl + "/ccpi1300_doReqSign.so", function () {
				// 저장 완료 후 ozd, pdf 저장
				var bundleNum = this.getParameter('BUNDLE_NUM');
				var contType  = this.getParameter("CONT_TYPE");
				if (opener) {
	...
	function doDelete() {
		EVF.confirm("${msg.M0013}", function () {
			...
			store.load(baseUrl + "/ccpi1300_doDeleteBundleContract.so", function () {
				EVF.alert("${msg.M0001}");
				if (opener) {
					opener.doSearch();
					doClose();
	...
	function doSavePdf() {
		...
		EVF.confirm("${msg.M0021 }", function () {
			...
			store.doFileUpload(function () {
				store.load(baseUrl + "/ccpi1300_doSavePdf.so", function () {
	...
	function doSearchForm() {
		...
		//store.load("/nhepro/CCTR/CCTA0030/ccta0030_doSearchMainForm.so", function () {
		  store.load(baseUrl + "/ccpi1300_doSearchMainForm.so", function () {
	...
	function doSearchSubForms() {
		...
		if (gridM.isExistsSelRow()) {
			...
			store.setGrid([gridA]);
			store.load(baseUrl + "/ccpi1300_doSearchAdditionalForm.so", function () {	
		...
		/* 		if(!gridM.isExistsSelRow()) {
				...
				if( CONT_NUM != "" && CONT_CNT != "" ) {
					if ("${form.PROGRESS_CD}" == "4300" && GRD_PROGRESSCD =="4300" ) {//4300:근로자 서명완료			
						...
						store.load(baseUrl + '/ccpi1300_doSelectPdfJsonData.so', function() {
...
/* 		var eformInputValue = "";
	...
	<e:window id="CCPI1300" onReady="init" initData="${initData}" title="${fullScreenName }" breadCrumbs="${breadCrumb }" margin="0 4px">
