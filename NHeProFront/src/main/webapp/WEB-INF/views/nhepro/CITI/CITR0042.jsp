...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    ...
    <script>
        var grid, gridDt;
        var gridTree;
        var baseUrl = "/nhepro/CITI/CITR0042/";
        var selRowId;
        ...
        function doSearchTree() {
            ...
            store.load(baseUrl + "citr0042_doSearchTree.so", function() {
        ...
        function doSearch() {
            ...
            var store = new EVF.Store();
            store.setGrid([grid]);
            store.load(baseUrl + "citr0042_doSearchGrid.so", function() {
            });
        }
        ...
        function doUpDown(type, btnFlag, rowIdx) {
            if( type == "up" ) {
                ...
                var selRowIdx = gridDt.getSelRowId();
                for(var x = selRowIdx.length - 1; x >= 0; x--) {
                	gridDt.delRow(selRowIdx[x]);
                }
            } else {
                var data;
                if(btnFlag) {
        ...
        function doChoose() {
            ...
            if(!EVF.isEmpty("${param.callType}") && "${param.callType}" == "S") {
                if (gridDt.getSelRowCount() > 1) { return EVF.alert("${msg.M0006}"); }
            }

            if(opener) {
                opener.window.focus();
                opener['${form.callbackFunction}'](gridDt.getSelRowValue(), "${form.rowIdx}");
            }
            else if(parent) {
                parent.window.focus();
                parent['${form.callbackFunction}'](gridDt.getSelRowValue(), "${form.rowIdx}");
            }
    ...
    <e:window id="CITR0042" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
        ...
        <e:panel id="rightPanel" width="74%">
            <e:searchPanel id="form" title="${msg.M9999}" labelWidth="${labelWidth}" columnCount="2" useTitleBar="false" onEnter="doSearch">
                ...
                <e:row>
                    ...
                    <%-- 품명/규격 --%>
                    <e:label for="ITEM_INFO" title="${form_ITEM_INFO_N}" />
                    <e:field>
                        <e:inputText id="ITEM_INFO" name="ITEM_INFO" value="" width="${form_ITEM_INFO_W}" maxLength="${form_ITEM_INFO_M}" disabled="${form_ITEM_INFO_D}" readOnly="${form_ITEM_INFO_RO}" required="${form_ITEM_INFO_R}" style="${imeMode}" maskType="${form_ITEM_INFO_MT}"/>
                    </e:field>
                </e:row>
                <e:row>
                    <%-- 제조사 --%>
