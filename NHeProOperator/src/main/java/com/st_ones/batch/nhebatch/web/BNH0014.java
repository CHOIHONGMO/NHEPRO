...
import com.st_ones.batch.nhebatch.service.BNH0014_Service;
...
@Service
public class BNH0014 extends EverJob {
    @Autowired private BNH0001_Service bnh0001_service;
    @Autowired private BNH0014_Service bnh0014_service;
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (super.isAllowedToRunBatch(context)) {
            ...
            bnh0014_service = SpringContextUtil.getBean(BNH0014_Service.class);

            String msg = "";
            int totalCount = 0;

            try {
                msg = bnh0014_service.doExecService(new HashMap<String, String>());
                jobRlt = "S";
            } catch (Exception e) {
            	...
            	if( telNums != null ) {
	            	for (String telNum : telNums) {
	                    try {
	    	                ...
	    	                smsMap.put("CONTENTS", "[전자구매Batch] 예가마감 사전안내(BNH0014) I/F시 오류가 발생");
                ...
                try {
                    ...
                    logData.put("JOB_ID", "BNH0014");
                    logData.put("JOB_NM", "예가마감 사전안내");
                    logData.put("JOB_KEY", "");
