...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
	<script>
		...
		var grid;
		var baseUrl = "/nhepro/CCTR/CCTR0051";
		var userFlag = false;
		...
		function init() {
			...
			grid.cellClickEvent(function (rowIdx, celName, value) {
				var param;
				var guar_type  = grid.getCellValue(rowIdx, "GUAR_TYPE");
		...
		function doGuarApp() {
			...
			EVF.confirm("신청하시겠습니까?", function() {
				...
				store.load(baseUrl + "/cctr0051_doSave.so", function () {
					return EVF.alert("${CCTR0051_0001}"); // 성공적으로 신청 되었습니다.
				});
			});
		}
		...
		function doSearch() {
			...
			store.setGrid([grid]);
			store.load(baseUrl + "/cctr0051_doSearch.so", function () {
                ...
                for(var i in allRowId) {
                    ...
                    var guar_type = grid.getCellValue(rowIdx, "GUAR_TYPE");
                    if (guar_type == "CONT") {
                    	grid.setCellReadOnly(rowIdx, "GUAR_TYPE2", true);
                    	grid.setCellReadOnly(rowIdx, "GUARANTEER", true);
                    	//grid.setCellReadOnly(rowIdx, "GUAR_APP", true);
                    }
                }
	...
	<e:window id="CCTR0051" initData="${initData}" onReady="init" title="${fullScreenName }" breadCrumbs="${breadCrumb }">
		...
		<e:inputHidden id="PAY_CNT" name="PAY_CNT" value="0"/>
