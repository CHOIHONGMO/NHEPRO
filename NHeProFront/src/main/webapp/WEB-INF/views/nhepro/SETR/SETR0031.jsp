...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    <script type="text/javascript">
        ...
        function doSave() {
            ...
            if(EVF.isEmpty(EVF.V("START_DATE"))) { return EVF.alert("${SETR0031_003 }"); }
            if(EVF.isEmpty(EVF.V("NOTICE_CONTENTS"))) { return EVF.alert("${SETR0031_002 }"); }

            EVF.confirm("${msg.M0021 }", function () {
                store.doFileUpload(function () {
                    store.load(baseUrl + "setr0031_doSave.so", function () {
                        var pNoticeNum = this.getParameter("NOTICE_NUM");
                        EVF.alert(this.getResponseMessage(), function () {
                            location.href = baseUrl + "SETR0031/view.so?NOTICE_NUM=" + pNoticeNum + "&detailView=false";
                            if (opener != null) {
                                opener.doSearch();
                            }
                        });
        ...
        function doDelete() {
            ...
            EVF.confirm("${msg.M0013 }", function () {
                store.load(baseUrl + "setr0031_doDelete.so", function () {
                    EVF.alert(this.getResponseMessage(), function() {
                        if (opener != null) {
                            opener.doSearch();
                        }
        ...
        function doClear() {
            EVF.confirm("${SETR0031_005 }", function () {
                location.href = baseUrl + "SETR0031/view.so?NOTICE_NUM=&detailView=false";
            });
        }
    ...
    <e:window id="SETR0031" onReady="init" initData="${initData}" title="${fullScreenName }" breadCrumbs="${breadCrumb }">
        <e:searchPanel id="form" useTitleBar="false" title="${form_CAPTION_N }" labelWidth="${longLabelWidth}" width="100%" columnCount="2">
            ...
            <e:row>
                ...
                <e:field>
                    <e:select id="VIEW_TYPE" name="VIEW_TYPE" value="${formData.VIEW_TYPE }" options="${viewTypeOptions}" width="${form_VIEW_TYPE_W}" disabled="${form_VIEW_TYPE_D}" readOnly="${form_VIEW_TYPE_RO}" required="${form_VIEW_TYPE_R}" placeHolder="" maskType="${form_VIEW_TYPE_MT}" />
                </e:field>
                <e:label for="BUYER_NM" title="${form_BUYER_NM_N}" />
                <e:field>
                    <e:inputText id="BUYER_NM" name="BUYER_NM" value="${formData.BUYER_NM }" width="${form_BUYER_NM_W}" maxLength="${form_BUYER_NM_M}" disabled="${form_BUYER_NM_D}" readOnly="${form_BUYER_NM_RO}" required="${form_BUYER_NM_R}" style="${imeMode}" maskType="${form_BUYER_NM_MT}"/>
                </e:field>
            </e:row>
            <e:row>
