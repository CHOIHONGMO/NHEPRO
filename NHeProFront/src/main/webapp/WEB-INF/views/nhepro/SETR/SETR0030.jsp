...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    <script type="text/javascript">
        ...
        function init() {
            ...
            grid.cellClickEvent(function(rowIdx, colIdx) {
                ...
                if(colIdx == "SUBJECT") {
                    ...
                    everPopup.openPopupByScreenId("SETR0031", 1000, 730, param);
            ...
            EVF.C("VIEW_TYPE").removeOption("USNE");
            EVF.C("VIEW_TYPE").removeOption("USXX");
        }
        
        function onIconClickVIEW_TYPE_S() {
            var param = {
                callBackFunction: "callBackVIEW_TYPE_S"
            };
            everPopup.openCommonPopup(param, "SP0066");
        }

        function callBackVIEW_TYPE_S(data) {
            EVF.V("VIEW_TYPE_S", data.CUST_NM);
        }
        ...
        function doSearch() {
            ...
            store.setGrid([grid]);
            store.load(baseUrl + "setr0030_doSearch.so", function() {
    ...
    <e:window id="SETR0030" initData="${initData}" onReady="init" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
        <e:searchPanel id="form" title="${form_CAPTION_N }" labelWidth="${labelWidth}" width="100%" columnCount="3" useTitleBar="false" onEnter="doSearch">
            <e:row>
                ...
                <e:field>
                    <e:select id="VIEW_TYPE" name="VIEW_TYPE" value="" options="${viewTypeOptions}" width="30%" disabled="${form_VIEW_TYPE_D}" readOnly="${form_VIEW_TYPE_RO}" required="${form_VIEW_TYPE_R}" placeHolder="" maskType="${form_VIEW_TYPE_MT}" />
                    <e:search id="VIEW_TYPE_S" name="VIEW_TYPE_S" value="" width="70%" maxLength="${form_VIEW_TYPE_S_M}" onIconClick="${form_VIEW_TYPE_S_RO ? 'everCommon.blank' : 'onIconClickVIEW_TYPE_S'}" disabled="${form_VIEW_TYPE_S_D}" readOnly="${form_VIEW_TYPE_S_RO}" required="${form_VIEW_TYPE_S_R}" maskType="${form_VIEW_TYPE_S_MT}" />
                </e:field>
