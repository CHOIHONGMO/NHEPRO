...
import com.st_ones.batch.nhebatch.service.BNH0019_Service;
...
@Service
public class BNH0019 extends EverJob {
    @Autowired private BNH0001_Service bnh0001_service;
    @Autowired private BNH0019_Service bnh0019_service;
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (super.isAllowedToRunBatch(context)) {
            ...
            bnh0019_service = SpringContextUtil.getBean(BNH0019_Service.class);

            String msg = "";
            int totalCount = 0;

            try {
                msg = bnh0019_service.doExecService(new HashMap<String, String>());
                jobRlt = "S";
            } catch (Exception e) {
            	...
            	if( telNums != null ) {
	            	for (String telNum : telNums) {
	                    try {
	    	                ...
	    	                smsMap.put("CONTENTS", "[전자구매Batch] 12개월 미접속 사용자 정보이관(BNH0019) I/F시 오류가 발생");
                ...
                try {
                    ...
                    logData.put("JOB_ID", "BNH0019");
                    logData.put("JOB_NM", "12개월 미접속 사용자 정보이관 및 72개월 미접속 삭제");
                    logData.put("JOB_KEY", "");
