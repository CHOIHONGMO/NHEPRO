...
import com.st_ones.batch.nhebatch.service.BNH0018_Service;
...
@Service
public class BNH0018 extends EverJob {
    @Autowired private BNH0001_Service bnh0001_service;
    @Autowired private BNH0018_Service bnh0018_service;
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (super.isAllowedToRunBatch(context)) {
            ...
            bnh0018_service = SpringContextUtil.getBean(BNH0018_Service.class);

            String msg = "";
            int totalCount = 0;

            try {
                msg = bnh0018_service.doExecService(new HashMap<String, String>());
                jobRlt = "S";
            } catch (Exception e) {
            	...
            	if( telNums != null ) {
	            	for (String telNum : telNums) {
	                    try {
	    	                ...
	    	                smsMap.put("CONTENTS", "[전자구매Batch] 11개월 장기 미사용자 휴면계정 전환_개인근로자(BNH0018) I/F시 오류가 발생");
                ...
                try {
                    ...
                    logData.put("JOB_ID", "BNH0018");
                    logData.put("JOB_NM", "11개월 장기 미사용자 휴면계정 전환 SMS 발송_개인근로자");
                    logData.put("JOB_KEY", "");
