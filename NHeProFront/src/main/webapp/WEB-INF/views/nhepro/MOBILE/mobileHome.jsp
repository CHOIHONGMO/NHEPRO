...
<script>
    ...
    function onChangeMonth() {
        var url = "/nhepro/MOBILE/getDay.so";
        var param = {
    ...
    // 근로계약서
    function doContract() {
        var url = "/nhepro/MOBILE/mobileHome_contract_list.so";
        $.post(url, {}, function (data) {
            ...
            for(var i in data) {
                contractHtml += '<tr>';
                contractHtml += '   <td><span class="txt-sgn-target">' + data[i].CODE_DESC + '</span></td>';
                contractHtml += '   <td>' + data[i].WORKER_CONT_DATE + '</td>';
                contractHtml += '   <td class="txt-left"><a href="javascript:doContractReport(' + i + ', \'cont\');">' + data[i].CONT_DESC + '</a></td>';
    ...
    function doContractReport(contNum, contType) {

        var contData = contractData[contNum];

        if(contData.CODE_DESC == '서명대기') {
            console.log(contData);

            var odiParamVal = "BUYER_CD=" + contData.BUYER_CD + ",CONT_NUM=" + contData.CONT_NUM + ",CONT_CNT=" + contData.CONT_CNT + ",WORKER_USER_ID=" + "${ses.userId}";
            ...
            var param = {
                ...
                WORKER_USER_ID: "${ses.userId}",
                // 파일명(개인근로계약 갑지)
                ozrName: "CCTR1270",
                // 서브 파일명
                SUB_FORM_FILE_NM: "",
                // ODI 명
                odiName: "CONT_USER_INFO",
                // OZ Scheduler Info
                serverUrl: "${ozServer}",
        ...
        } else if(contData.CODE_DESC == '계약완료') {
    ...
    function contract_reload(data) {
        // 계약완료 처리
        var url = "/nhepro/MOBILE/mobile_doContract.so";
        var param = {
    ...
    // 서약서
    function doPledge() {
        var url = "/nhepro/MOBILE/mobileHome_doPledge_list.so";
        $.post(url, {}, function (data) {
            pledgeData = data;
            var contractHtml = "";
            ...
            for(var i in data) {
                contractHtml += '<tr>';
                contractHtml += '   <td class="txt-left"><a href="javascript:doPdfPage(' + i + ', \'security\');">' + data[i].CODE_DESC + '</a></td>';
                contractHtml += '   <td>' + data[i].SIGN_DATE + '</td>';
                contractHtml += '   <td><a href="javascript:downloadFile(\''+data[i].UUID+'\', \''+data[i].UUID_SQ+'\', \'security\')"><img src="/images/nhepro/mobile/m-file.png" alt="첨부파일" class="ico-file"></a></td>';
                contractHtml += '</tr>';
            }

            $('#pledge-list tbody').html(contractHtml);
        }, "json");
    }

    // 공지사항
    function doNotice() {
        var url = "/nhepro/MOBILE/mobileHome_doNotice_list.so";
        $.post(url, {}, function (data) {
            var contractHtml = "";
            for(var i in data) {
                contractHtml += '<tr>';
    ...
    // 회원정보
    function doMember() {
        var url = "/nhepro/MOBILE/mobileHome_doMember.so";
    ...
    function doNoticePage(notice_num) {
        // 공지사항 상세
        var url = "/nhepro/MOBILE/MECM_060/view.so";
        var param = {
    ...
    function perUser() {
        var url = '/nhepro/MOBILE/MAGG_030/view.so';
        var param = {
...
<div class="page-wrap">
    ...
    <section class="contents">
        <div class="tabs-box home">
            <ul class="tabs-menu">
                <li><a href="#contract-list" onclick="doContract();">근로계약서</a></li>
                <li><a href="#pledge-list" onclick="doPledge();">서약서</a></li>
                <li><a href="#notice-list" onclick="doNotice();">공지사항</a></li>
                <li><a href="#member-info" onclick="doMember();">회원정보</a></li>
            </ul>
            <div class="tabs-contents">
                ...
                <div class="cont" id="pledge-list">

                    <div class="table-header-fixed">
                        <div class="data-table">
                            <div class="header-bg"></div>
                            <table class="list">
                                <caption>서약서 목록</caption>
                                <colgroup>
                                    <col style="width:65%"/>
                                    <col style="width:23%"/>
                                    <col style="width:12%"/>
                                ...
                                <tr>
                                    <th scope="col">
                                        <div class="th-fx01">서약서명</div>
                                    </th>
                                    <th scope="col">
                                        <div class="th-fx02">서명일자</div>
                                    </th>
                                    <th scope="col">
                                        <div class="th-fx03">PDF</div>
                                    </th>
                                </tr>
                                </thead>
                                <tbody>
                    ...
                    <section class="fbox sign-up">
                        <fieldset class="ins">
                            <legend class="v-hidden">회원가입</legend>
                                ...
                                <div class="t-row">
                                    <div class="t-cell01"><label for="PASSWORD"><i class="title"></i>비밀번호</label></div>
                                    <div class="t-cell02">
...
<script>
    $(function () {
        ...
        $('#email-btn').on('click', function (e) {
            ...
            if(emailValidFlag) {
                var url = "/nhepro/MOBILE/emailCheck.so";
                var param = {
                    USER_ID: $('#USER_ID').val(),
                    EMAIL: $('#EMAIL').val()
                };
        ...
        $('#certified_send_btn').on('click', function (e) {
            ...
            // 인증 시간 기록
            var url = "/nhepro/MOBILE/certified_update.so";
            var param = {
        ...
        $('#certified_confirm_btn').on('click', function (e) {
            ...
            if (certifiedFlag) {
                // 인증 시간 기록
                var url = "/nhepro/MOBILE/certified_confirm.so";
                var param = {
                    CELL_NUM: $('#CELL_NUM1').val() + "-" + $('#CELL_NUM2').val() + "-" + $('#CELL_NUM3').val(),
        ...
        $('#doSave').on('click', function () {
            ...
            var url = "/nhepro/MOBILE/doSave.so";
