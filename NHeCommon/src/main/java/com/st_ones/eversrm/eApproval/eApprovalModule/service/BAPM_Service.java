...
public class BAPM_Service extends BaseService {
	...
	public void doApprovalProcess(Map<String, String> docInfo, String strApprovalFormData, String strApprovalGridData) throws Exception {
        ...
        if(docInfo.get("BUYER_CD") == null || EverString.nullToEmptyString(docInfo.get("BUYER_CD")).equals("")) {
            if(userInfo != null) { docInfo.put("BUYER_CD", userInfo.getCompanyCd()); }
        }
		...
		try {
			...
			approvalHeader.put("BUYER_CD", docInfo.get("BUYER_CD"));
			Map<String, String> sctmInfo = eApprovalMapper.selectSTOCSCTM(approvalHeader);
	        ...
	        mailMap.put("REF_MODULE_CD", "MSIGN01");
	        mailMap.put("RECV_USER_ID", nextSignUserId);
	        mailMap.put("REF_NUM", docInfo.get("APP_DOC_NUM"));
	        //개발테스트시에는 잠시 주석
            ...
            smsMap.put("CONTENTS", "[전자구매시스템]" + sctmInfo.get("REG_USER_NM") + " "+sctmInfo.get("DUTY_NM") + " 님이 상신한 결재문서가 도착하였습니다");
	        smsMap.put("REF_MODULE_CD", "SSIGN01");
	        smsMap.put("REF_NUM", docInfo.get("APP_DOC_NUM"));
	        smsMap.put("RECV_USER_ID", nextSignUserId);
		    ...
		    logger.error("결재문서 상신 메일&문자 발송 오류 : " + ex.getMessage());
		}
	}

	/**
	 * HMCHOI-151027
	 * G/W 연동시 STOCSCTM 테이블에 데이터를 등록함
	 * @param docInfo APP_DOC_NUM, APP_DOC_CNT, DOC_TYPE, PROCEEDING_FLAG를 설정 해야 합니다.
	 * @throws ApprovalException
	 * @throws Exception
	 */
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void doApprovalProcess(Map<String, String> docInfo) throws Exception {
		bapm_Mapper.insertSTOCSCTM(docInfo);
	}
	...
	public String cancelApproval(Map<String, String> map) throws Exception {
		...
		String buyerCd 	 = map.get("BUYER_CD");
		String appDocNum = map.get("APP_DOC_NUM");
		String appDocCnt = map.get("APP_DOC_CNT");

		return endApprovalService.doAfterCancel(docType, buyerCd, appDocNum, appDocCnt);
	}
	...
	*/
	...
	public List<Map<String, Object>> singerList(Map<String, String> param) throws Exception {
		return bapm_Mapper.singerList(param);
	}

	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public String putBkCost(Map<String, String> param) throws Exception {

		// 테스트를 위해 값 변경.
		if( PropertiesManager.getBoolean("eversrm.system.localserver") || PropertiesManager.getBoolean("eversrm.system.developmentFlag") ) {
//			param.put("CORP_NO", "2148206195");
//			param.put("BRC", "005085");
//			param.put("USER_ID", "11303733");
        } // 운영에서 CUSTOMER 계정은 하드코딩...
		else {
//			if(EverString.nullToEmptyString(param.get("USER_ID")).equals("CUSTOMER")) {
//				param.put("CORP_NO", "2148206195");
//				param.put("BRC", "005085");
//				param.put("USER_ID", "11303733");
//			}
		...
		// EPRO_WRS_DS = '30' 또는 '40'일 때 반드시 계약금액을 넣어야 함.
		if(param.get("EPRO_WRS_DS").equals("30") || param.get("EPRO_WRS_DS").equals("40")) {
			if(param.get("tmp") == null || param.get("tmp").equals("")) {
				param.put("tmp", "0");
			}
		}
         ...
         * )
         */
		bapm_Mapper.putBkCost(param);
		// 2021.01.12 사용수수료 청구 후 결과값 return
		if( !param.get("RESULT_STR").equals("OK") ) {
			return "사용 수수료 부과시 오류 발생 => " + param.get("RESULT_STR") + " 관리자 > 이용권구매 메뉴를 통해 이용권을 구매하세요.";
		}
	...
	private void eApprovalMailSend(String approvalStatus, Map<String, String> receiverInfo, String approvalNum, String appDocCnt) throws Exception {
		...
		if(!receiverInfo.get("RECV_EMAIL").equals("")) {
