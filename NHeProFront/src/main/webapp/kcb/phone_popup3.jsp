...
<%
    ...
    //'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
	String CP_CD = "V45030000000";	// 회원사코드
...
<script type="text/javascript" >
	function fncOpenerSubmit() {
		...
		var param = {
			...
			RSLT_MSG: "<%=RSLT_MSG%>",
			RSLT_NAME: "<%=RSLT_NAME%>",
			RSLT_BIRTHDAY: "<%=RSLT_BIRTHDAY%>",
			RSLT_SEX_CD: "<%=RSLT_SEX_CD%>",
			RSLT_NTV_FRNR_CD: "<%=RSLT_NTV_FRNR_CD%>",
			DI: "<%=DI%>",
			CI: "<%=CI%>",
			CI_UPDATE: "<%=CI_UPDATE%>",
		...
		if(opener) {
			window.opener["kcbCallbackFunction"](param);
		}
		// IE 미작동 -> 153 line으로 수정
		//self.close();
