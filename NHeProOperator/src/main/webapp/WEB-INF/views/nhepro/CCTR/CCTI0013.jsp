...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
	...
	<script type="text/javascript">
		...
		var grid = {};
		var baseUrl = "/nhepro/CCTR/CCTI0013";
	    ...
	    function doSearchECCR() {
	        ...
	        store.setGrid([grid]);
	        store.load(baseUrl + "/ccti0013_doSearchECCR.so", function() {
		...
		function doSave() {
			...
			if(CKEDITOR.instances.cont_content.getData() == "") {
				return EVF.alert("${CCTI0013_0005}");
			}
			--%>
			...
			if( curSignStatus == 'E' ){
				return EVF.alert("${CCTI0013_0007}");
    		} else {
    			if( signStatus == "T" ) {
        			msg = "${msg.M0021}";
    			} else {
		...
		function signSaveT(msg) {
			...
			EVF.confirm(msg, function () {
				...
				store.doFileUpload(function() {
					store.load(baseUrl + '/ccti0013_doSave.so', function() {
		...
		function doReqSign() {
    		...
    		if( signStatus == 'T' ){
    			EVF.confirm("${CCTI0013_0004}", function () {
	...
	<e:window id="CCTI0013" onReady="init" initData="${initData}" title="${fullScreenName }" breadCrumbs="${breadCrumb }" margin="0 4px">
