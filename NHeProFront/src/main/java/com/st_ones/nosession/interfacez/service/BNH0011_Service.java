...
import com.st_ones.nosession.interfacez.BNH0011_Mapper;

@Service(value = "BNH0011_Service")
public class BNH0011_Service extends BaseService {
	...
	public String doExecService(Map<String, String> param) throws Exception {
		System.err.println("========================================== ITPortal 전송 시작 ===================================================");
		...
		// 2021.09.07 : 계약건 다수 대상 가져오기
        Map<String, Object> formData = new HashMap<String, Object>(param);
        String contNumCnt = param.get("CONT_NUM_CNT");
        if (contNumCnt != null && !contNumCnt.isEmpty()) {
            formData.put("CONT_NUM_LIST", Arrays.asList(contNumCnt.split(",")));
        } else {
            formData.put("CONT_NUM_LIST", new ArrayList<String>());
        }
    	
		List<Map<String, String>> contList = bnh0011mapper.getTragetContList(formData);
        if (contList != null) {
		...
		if(ecctListITA.size()!=0 || ecctListITB.size()!=0) {
            ...
            if (fileSq != null && fileSq.length() > 4) {
                fileInfo.put("SEQ", fileSq.substring(4));
            } else {
                fileInfo.put("SEQ", fileSq);
            }
    ...
    public Map<String,Object> makeData( Map<String,String> param ) throws Exception {
    	...
    	if (ecmtData == null || ecmtData.size() == 0) return null;
    	Map<String,String> ecct = bnh0011mapper.makeEcct(param);
        if (ecct == null) return null;
        ...
        long CONT_AMT_5 = getSafeLong(ecct.get("5_CONT_AMT"));
        long CONT_AMT_1 = getSafeLong(ecct.get("1_CONT_AMT"));
		...
		if ( targetIdList != null && targetIdList.size() != 0) {
            Map<String, String> targetIdMap = new HashMap<String, String>();
            for (Map<String, String> map : targetIdList) {
                targetIdMap.put(map.get("CODE"), map.get("CODE_DESC"));
            }
			changeRegUserId(ecct, targetIdMap);
	    	changeRegUserId(ecmt, targetIdMap);
	    	changeRegUserId(ecpc, targetIdMap);
	    	changeRegUserId(ecpy, targetIdMap);
	    	changeRegUserId(ecbv, targetIdMap);
	    	changeRegUserId(atch, targetIdMap);
		}
    ...
    private void changeBizType(List<Map<String,String>> atchMap, String ECNum, String ECCnt){
    	if (atchMap == null || atchMap.isEmpty()) {
            return;
        }
    	for( Map<String,String> target : atchMap) {
    		...
    		String realFileNm = companyCd + ECNum + ECCnt + "." + fileExtension;
    		
    		// ECE로 바꿔야하는 파일인 경우 (계약서)
    		if("EC".equals(target.get("BIZ_TYPE")) && realFileNm.equals(target.get("REAL_FILE_NM"))) {
    			target.put("BIZ_TYPE", "ECE");
    		}
		}
    }
    
    private void changeRegUserId(Map<String, String> param, Map<String, String> targetIdMap) {
        if (param == null || targetIdMap == null || targetIdMap.isEmpty()) {
            return;
        }
        Map<String, String> updates = new HashMap<String, String>();
        for (Map.Entry<String, String> entry : param.entrySet()) {
            String value = entry.getValue();
            if (targetIdMap.containsKey(value)) {
                updates.put(entry.getKey(), targetIdMap.get(value));
            }
        }
        if (!updates.isEmpty()) {
            param.putAll(updates);
        }
	}

	private void changeRegUserId(List<Map<String, String>> param, Map<String, String> targetIdMap) {
		if (param == null || param.isEmpty() || targetIdMap == null || targetIdMap.isEmpty()) {
            return;
        }
		for (Map<String, String> item : param) {
            changeRegUserId(item, targetIdMap);
		}
	}

	public void makeFile(String type, Map<String, String> fileInfo, List<Map<String,String>> ecctList, List<Map<String,String>> ecmtList, List<Map<String,String>> ecpcList,
			List<Map<String,String>> ecpyList, List<Map<String,String>> ecbvList, List<Map<String,String>> atchList) throws Exception
	{
        if (ecctList == null || ecctList.isEmpty()) {
            return;
        }
        ...
        if(ecmtList != null && !ecmtList.isEmpty()) {
            String[] ecmtArray = {
        ...
        if(ecpcList != null && !ecpcList.isEmpty()) {
            String[] ecpcArray = {
        ...
        if(ecpyList != null && !ecpyList.isEmpty()) {
            String[] ecpyArray = {
        ...
        if(ecbvList != null && !ecbvList.isEmpty()) {
            String[] ecbvArray = {
	...
	public void copyFile(String sourceFileName, String targetFileName,int lineCou,String totime) throws Exception {
		...
		System.err.println(sourceFileName+"=======================copyFile====================="+targetFileName);
		try {
	        String completeFolder = PropertiesManager.getString("eversrm.interface.folder.BNH0011");
	        ...
	        if (sourceFile.exists()) {
                try (FileInputStream fis = new FileInputStream(sourceFile);
                    ...
                    int cou;
                    while ((cou = fis.read(buf)) > 0) {
                        fos.write(buf, 0, cou);
                    }
                }

				String chkText = sourceFile.getName()+"|"+sourceFile.length()+"|"+totime+"|"+lineCou+"||||||";
				String chkFileName = completeFolder + sourceFile.getName().substring(0,sourceFile.getName().lastIndexOf(".")) + ".chk";
				File chkFile = new File(chkFileName);
                try (FileWriter fw = new FileWriter(chkFile)) {
                    fw.write(chkText);
                }
	        }
		} catch (IOException e) {
