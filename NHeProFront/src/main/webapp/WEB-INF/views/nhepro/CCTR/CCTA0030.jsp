...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
	...
	<script type="text/javascript">
		...
		var baseUrl  = "/nhepro/CCTR/CCTA0030";
		...
		function init() {
		    ...
		    gridECMT.setColGroup([
                ...
                ,{
                    ...
                    "columns": ['PRE_CONT_NUM', 'PRE_CONT_CNT', 'CM_REQ_ID', 'MULPUM_ID', 'IF_TYPE']
                }
            ],50);
		...
		function doSearchCNDT() {
			...
			store.load(baseUrl + '/ccta0030_doSearchCNDT.so', function() {
		...
		function callBackHD_PY_USER_NM(data) {
			...
			if(EVF.V("CONT_REQ_CD") == "10" ) {
				...
				var pyBuyerCd = gridECPC_HD.getCellValue(data.rowIdx, "PY_BUYER_CD");
				if( pyBuyerCd == '' || pyBuyerCd == null ) {
					if(check) {
						return EVF.alert("동일한 지불고객사가 있습니다.");
					}
				}
		...
		function callBackINSPECT_USER_ID(data) {
			...
			if (selectedRow.length > 1) {
				EVF.confirm('${CCTA0030_0035}', function() {
		...
		function doSearchECCM() {
			...
			store.load(baseUrl + '/ccta0030_doSearchECCM.so', function() {
		...
		function doSearchECMT() {
			var store = new EVF.Store();
			store.setGrid([gridECMT]);
			store.load(baseUrl + '/ccta0030_doSearchECMT.so', function() {
		...
		function doSearchForm() {
			...
			store.setGrid([ gridM ]);
			store.load(baseUrl + '/ccta0030_doSearchMainForm.so', function() {
		...
		function doSearchSubForms() {
			if(gridM.isExistsSelRow()) {
				...
				store.setGrid([ gridA ]);
				store.load(baseUrl + '/ccta0030_doSearchAdditionalForm.so', function() {
		...
		function doSave() {
			...
			if( payPercent > 100 || payPercent < 100 ) {
            	return EVF.alert("${CCTA0030_0034}");
            }
			...
			EVF.confirm("${msg.M0021 }", function() {
				...
				store.doFileUpload(function() {
					store.load(baseUrl + '/ccta0030_doSave.so', function() {
						// 저장 완료 후 ozd, pdf 저장
		...
		function doVendorSend() {
			...
			EVF.confirm("변경된 내용이 있는 경우 '저장' 이후에 협력사와 공유하세요.\n\n작성중인 계약서를 협력사와 공유하시겠습니까?", function() {
				store.load(baseUrl + '/ccta0030_doVendorSend.so', function() {
						...
						for(var j in parseEcpcHdInfo) {
							...
							$.ajax({
								...
								success: function(data) {
									...
									$.ajax({
										url: baseUrl + "/ccta0030_doUpdatePdfUUID.so",
										type: "post",
										data: {json: data},
										async: false,
										success: function(data) {
		...
		function doReqSign() {
			...
			store.getGridData(gridECMT, 'all'); // 품목정보
			store.doFileUpload(function() {
				store.load(baseUrl + '/ccta0030_doReqSign.so', function() {
					// 저장 완료 후 ozd, pdf 저장
					var contNum = this.getParameter('CONT_NUM');
					var contCnt = this.getParameter('CONT_CNT');
					if(EVF.V("CONT_NUM") == "" && EVF.V("CONT_CNT") == "") {
		...
		<%-- 계약서를 삭제처리한다 --%>
		function doDelete() {
			EVF.confirm("${msg.M0013}" + "\n" + "${CCTA0030_0042}", function() {
				var store = new EVF.Store();
				store.load(baseUrl + "/ccta0030_doDeleteContract.so", function () {
		...
		function doReSend() {
			EVF.confirm("${CCTA0030_0041}", function() {
				EVF.confirm("${CCTA0030_0040}", function() {
					var store = new EVF.Store();
					store.load(baseUrl + "/ccta0030_doReSend.so", function () {
						EVF.confirm(this.getResponseMessage(), function () {
							if (opener) {
								opener['doSearch']();
								doClose();
		...
		function doSign() {
			...
			EVF.confirm("${CCTA0030_0016}", function() {
				var store = new EVF.Store();
				store.load(baseUrl + '/ccta0030_guarCancelData.so', function() {
		...
		function signCompleteCallback() {
			...
			store.doFileUpload(function() {
				...
				store.load(baseUrl + '/ccta0030_doSign.so', function () {
		...
		/**
		 * 2020.12.16
		 * 계약서작성화면 계약번호 클릭 시 ccta0030_doUpdatePdfUUID 업데이트 시 고객사정보 잘못 가져오는 오류로 인해 업데이트 실패 수정
		 */
		function eformCallbackFunction(json, jsonData) {
		    ...
		    store.load(baseUrl + '/ccta0030_doUpdateEformJsonData.so', function() {
			    // pdf 이동
			    var maxFileCnt = 0;
			    var allRowId = gridECCM.getAllRowId();
			    for(var i in allRowId) {
	...
	<e:window id="CCTA0030" onReady="init" initData="${initData}" title="${fullScreenName }" breadCrumbs="${breadCrumb }" margin="0 4px">
		<!-- 2021.07.16 : 계약서 작성유형(PR : 구매의뢰, CN : 품의) -->
		<e:inputHidden id='CREATE_TYPE' name='CREATE_TYPE' value="${form.CREATE_TYPE}"/>
		<!-- 2023.11.16 : 뇽협은행IT부문 계약대기현황에서 작성한 연장계약에 대한 구분  -->
		<e:inputHidden id='AUTO_EXTEND' name='AUTO_EXTEND' value="${param.autoExtend}"/>
		...
		<e:searchPanel id="form" title="계약정보" columnCount="3" labelWidth="135" onEnter="doSearch" useTitleBar="false">
			...
			<e:row>
				...
				<e:field>
					...
					<e:text>${CCTA0030_STAMP_DUTY_TXT} / </e:text>
		...
		<e:panel id="panel_01" width="100%">
			<e:title title="${CCTA0030_PAY_TITLE}" />
