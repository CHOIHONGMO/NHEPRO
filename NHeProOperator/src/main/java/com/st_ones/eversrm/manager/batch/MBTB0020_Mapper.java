package com.st_ones.eversrm.manager.batch;

import org.springframework.stereotype.Repository;

import java.util.List;
import java.util.Map;

/**
 * <pre>
 ******************************************************************************
 * 상기 프로그램에 대한 저작권을 포함한 지적재산권은 ㈜에스티원즈에 있으며,
 * ㈜에스티원즈가 명시적으로 허용하지 않은 사용, 복사, 변경, 제3자에의 공개, 배포는 엄격히 금지되며,
 * ㈜에스티원즈의 지적재산권 침해에 해당됩니다.
 * (Copyright ⓒ 2013 ST-ONES CORP., ALL RIGHTS RESERVED | Confidential)
 ******************************************************************************
 * </pre>
 * @File Name : MBTB0020_Mapper.java
 * @date 2013. 07. 22.
 * @version 1.0
 */

@Repository
public interface MBTB0020_Mapper {

	/**
	 * 화면명 : BATCH 실행
	 * 처리내용 : 시스템에 등록된 Batch를 수동으로 실행시키는 화면.
	 * 경로 : 시스템관리 > 시스템 > BATCH 실행
	 */
	List<Map<String, String>> getBatchManagerSms(Map<String, String> param);

	int doSaveBatchLog(Map<String, Object> gridData);

	List<Map<String, Object>> doSearchBatchLogList(Map<String, String> param);
	
	List<Map<String, Object>> doSearchClobGroup(Map<String, String> param);
	
	List<Map<String, Object>> doSearchClobSub(Map<String, Object> param);
	
	void doChangeClob(Map<String, String> param);

}
