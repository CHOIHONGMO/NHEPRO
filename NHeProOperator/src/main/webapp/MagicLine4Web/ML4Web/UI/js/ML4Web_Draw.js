	...
	loadCSS : function(){
		...
		for( var i = 0; i < ML4WebApi.webConfig.storageList.length; i++){
			if( ML4WebApi.webConfig.storageList[i] === "unisign" ){
				...
				{
					document.createStyleSheet(contextPath + '/MagicLine4Web/ML4Web/UI/css/ML_common_nts.css?v=1');	
					...
					if( isMobile ){					
						...
						$('head').append( $('<link rel="stylesheet" type="text/css" />').attr('href', contextPath + '/MagicLine4Web/ML4Web/UI/css/ML_common_nts.css?v=1' ) );
					}
	...
	initWebMainEvent : function() {
		...
		$('[id^="stg_"]').click(function () {
			...
			if(stg_id == "web" || stg_id == "kftc" || stg_id == "web_kftc"){
				...
				$("#get_cert").html("<img src='UI/images/icon_search.png' alt='브라우저'>"+$.i18n.prop("TS035"));				
				...
				$("#ML_window").css('width', '418px');
				// 공인인증서 가져오기 문구 css
				$("#get_cert").html("<img src='UI/images/icon_search.png' alt='브라우저'>"+$.i18n.prop("TS036"));
 ...
 */
	browser_menu : function() {
		...
		child += '<img src="UI/images/browser_menual3.png" style="width:250px; heigth:550px;" alt="브라우저 인증서 사용방법">';
		child += '<img id="manual_close" src="UI/images/btn_hd_close.png" style="width:20px; heigth:20px;position:absolute; left:230px; " alt="브라우저 인증서 사용방법 닫기버튼" >';
		child += '</div>';
		
		if ( $("#browser_manual1").length > 0 ) {
			//$('#ML_Dialog_common').MLjquiWindow('destroy');
			var select = document.getElementById('ML_window');
			select.removeChild(select.lastChild);
		}
		
		var node = document.createElement("div");
		node.innerHTML=child;
		document.getElementById('ML_window').appendChild(node);
	            ...
	            columns: [
	              { text: $.i18n.prop("TS063"), dataField: 'sectionValues', cellsAlign: 'left', align: 'center', cellsFormat: 'c2',
	                     cellsRenderer: function (row, column, value, rowData) {
	                          ...
	                          var img = '<img width="80" height="98" style="display:block;" src="' + imgurl + '"/>';
	                          image += img;
	                          image += "</div>";
				...
				columns: [
					{ text: $.i18n.prop("TS059"), dataField: 'Cn', width: 140, align: 'center',
						cellsRenderer: function (row, column, value, rowData) {
							...
							var img = '<img width="16" height="16" style="display:block;" src="' + imgurl + '" alt="인증서"/>';
							image += img;
...
// 에러 alert dialog 처리
	...
	confirm : function( message, callback ){
		...
		DSDialog.openConfirm(popOption, function(code,jsonObj){
...
var ML4WebSaveCertDraw = {
	...
	_createWindow : function() {
		...
		var xValue = offset.left + (($(window).width()/2)-(window_width/2));
		var position = { x: xValue, y: offset.top + 50};
		var popupPosition =  {x: offset.left + ($(window).width()/2) - 185, y: offset.top + 200};
