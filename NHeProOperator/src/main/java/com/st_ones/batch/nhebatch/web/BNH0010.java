...
@Service
public class BNH0010 extends EverJob {
    @Autowired private BNH0001_Service bnh0001_service;
    @Autowired private BNH0010_Service bnh0010_service;
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (super.isAllowedToRunBatch(context)) {
            ...
            bnh0010_service = SpringContextUtil.getBean(BNH0010_Service.class);

            String msg = "";
            int totalCount = 0;

            try {
                msg = bnh0010_service.doExecService(new HashMap<String, String>());
                jobRlt = "S";
            } catch (Exception e) {
            	...
            	if( telNums != null ) {
	            	for (String telNum : telNums) {
	                    try {
	    	                ...
	    	                smsMap.put("CONTENTS", "[전자구매Batch] IT-Portal 계약의뢰(BNH0010) I/F시 오류가 발생");
                ...
                try {
                    ...
                    logData.put("JOB_ID", "BNH0010");
                    logData.put("JOB_NM", "IT Portal 계약의뢰 수신");
                    logData.put("JOB_KEY", "");
