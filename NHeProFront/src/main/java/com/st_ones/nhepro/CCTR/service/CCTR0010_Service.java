...
public class CCTR0010_Service {
    ...
    // 서식 저장 및 결재상신
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    public Map<String, String> ccti0011_doSave(Map<String, String> formData, List<Map<String, Object>> grid) throws Exception {
        UserInfo userInfo = UserInfoManager.getUserInfo();
    ...
    // 부서식 저장
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    public void ccti0011_doSubSave(List<Map<String, Object>> grid) throws Exception {
        for (Map<String, Object> gridData : grid) {
            gridData.put("updateRelFormSeq", "true");
            cctr0010_Mapper.newFormRegistrationDoUpdateGridData(gridData);
        }
