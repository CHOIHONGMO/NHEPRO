...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    <script type="text/javascript">
        ...
        function init() {
            ...
            EVF.C("VIEW_TYPE").removeOption("USNA");
            EVF.C("VIEW_TYPE").removeOption("USNI");
            EVF.C("VIEW_TYPE").removeOption("USNC");

            onChangeVIEW_TYPE();
        }

        function onChangeVIEW_TYPE() {
            if(EVF.V("VIEW_TYPE") == "USXX") {
                EVF.C("VENDOR_NM").setRequired(true);
            } else {
                EVF.C("VENDOR_NM").setRequired(false);
                EVF.V("VENDOR_CD", "");
                EVF.V("VENDOR_NM", "");
            }
        }

        function onIconClickVENDOR_NM() {
            if(EVF.V("VIEW_TYPE") == "USXX") {
                var param = {
                    callBackFunction: "callBackVENDOR_NM"
                };
                everPopup.openCommonPopup(param, "MP0001");
            }
        }

        function callBackVENDOR_NM(data) {
            var vendor_cd = "";
            var vendor_nm = "";
            ...
            for(var idx in data) {
                if(idx == 0) {
                    ...
                    vendor_cd = vendor_cd + "," + data[idx].VENDOR_CD;
                    vendor_nm = vendor_nm + "," + data[idx].VENDOR_NM;
                }
            }

            EVF.V("VENDOR_CD", vendor_cd);
            EVF.V("VENDOR_NM", vendor_nm);
        }
        ...
        function doSave() {
            ...
            if(EVF.isEmpty(EVF.V("START_DATE"))) { return EVF.alert("${CETR0031_003 }"); }
            if(EVF.isEmpty(EVF.V("NOTICE_CONTENTS"))) { return EVF.alert("${CETR0031_002 }"); }

            EVF.confirm("${msg.M0021 }", function () {
                store.doFileUpload(function () {
                    store.load(baseUrl + "cetr0031_doSave.so", function () {
                        var pNoticeNum = this.getParameter("NOTICE_NUM");
                        EVF.alert(this.getResponseMessage(), function () {
                            // location.href = baseUrl + "CETR0031/view.so?NOTICE_NUM=" + pNoticeNum + "&detailView=false";
                            if (opener != null) {
                                opener.doSearch();
                            }
        ...
        function doDelete() {
            ...
            EVF.confirm("${msg.M0013 }", function () {
                store.load(baseUrl + "cetr0031_doDelete.so", function () {
                    EVF.alert(this.getResponseMessage(), function() {
                        if (opener != null) {
                            opener.doSearch();
                        }
        ...
        function doClear() {
            EVF.confirm("${CETR0031_005 }", function () {
                location.href = baseUrl + "CETR0031/view.so?NOTICE_NUM=&detailView=false";
            });
        }
    ...
    <e:window id="CETR0031" onReady="init" initData="${initData}" title="${fullScreenName }" breadCrumbs="${breadCrumb }">
        <e:searchPanel id="form" useTitleBar="false" title="${form_CAPTION_N }" labelWidth="${longLabelWidth}" width="100%" columnCount="2">
            ...
            <e:row>
                ...
                <e:field>
                    <e:select id="VIEW_TYPE" name="VIEW_TYPE" value="${formData.VIEW_TYPE }" options="${viewTypeOptions}" onChange="onChangeVIEW_TYPE" width="${form_VIEW_TYPE_W}" disabled="${form_VIEW_TYPE_D}" readOnly="${form_VIEW_TYPE_RO}" required="${form_VIEW_TYPE_R}" placeHolder="" usePlaceHolder="false" maskType="${form_VIEW_TYPE_MT}" />
                </e:field>
                <e:label for="VENDOR_NM" title="${form_VENDOR_NM_N}"/>
                <e:field>
                    <e:search id="VENDOR_NM" name="VENDOR_NM" value="${formData.VENDOR_NM }" width="${form_VENDOR_NM_W}" maxLength="${form_VENDOR_NM_M}" onIconClick="onIconClickVENDOR_NM" disabled="${form_VENDOR_NM_D}" readOnly="${form_VENDOR_NM_RO}" required="${form_VENDOR_NM_R}" maskType="${form_VENDOR_NM_MT}" />
            ...
            <e:row>
                ...
                <e:field>
                    ...
                    <e:inputText id="REG_USER_NM" name="REG_USER_NM" value="${formData.REG_USER_NM }" width="${REG_USER_NM_W}" maxLength="${REG_USER_NM_M}" disabled="${form_REG_USER_NM_D}" readOnly="${form_REG_USER_NM_RO}" required="${form_REG_USER_NM_R}" style="${imeMode}" maskType="${form_REG_USER_NM_MT}"/>
                </e:field>
            </e:row>
            <e:row>
                <e:label for="NOTICE_CONTENTS" title="${form_NOTICE_CONTENTS_N }" />
                <e:field colSpan="3">
                    <e:richTextEditor id="NOTICE_CONTENTS" name="NOTICE_CONTENTS" width="${form_NOTICE_CONTENTS_W }" height="1000px" required="${form_NOTICE_CONTENTS_R }" readOnly="${form_NOTICE_CONTENTS_RO }" disabled="${form_NOTICE_CONTENTS_D }" value="${formData.NOTICE_CONTENTS }" useToolbar="${!param.detailView}" />
                </e:field>
            </e:row>
            <e:row>
