...
import com.st_ones.batch.nhebatch.service.BNH0020_Service;
...
@Service
public class BNH0020 extends EverJob {
    @Autowired private BNH0001_Service bnh0001_service;
    @Autowired private BNH0020_Service bnh0020_service;
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (super.isAllowedToRunBatch(context)) {
            ...
            bnh0020_service = SpringContextUtil.getBean(BNH0020_Service.class);

            String msg = "";
            int totalCount = 0;

            try {
                msg = bnh0020_service.doExecService(new HashMap<String, String>());
                jobRlt = "S";
            } catch (Exception e) {
            	...
            	if( telNums != null ) {
	            	for (String telNum : telNums) {
	                    try {
	    	                ...
	    	                smsMap.put("CONTENTS", "[전자구매Batch] 검수 및 대금지급 지연 안내(BNH0020) I/F시 오류가 발생");
                ...
                try {
                    ...
                    logData.put("JOB_ID", "BNH0020");
                    logData.put("JOB_NM", "검수 및 대금지급 지연 안내");
                    logData.put("JOB_KEY", "");
