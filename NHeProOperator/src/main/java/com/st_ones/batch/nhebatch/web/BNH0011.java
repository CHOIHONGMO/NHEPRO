...
@Service
public class BNH0011 extends EverJob {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (super.isAllowedToRunBatch(context)) {
            ...
            bnh0011_service = SpringContextUtil.getBean(BNH0011_Service.class);

            String msg = "";
            int totalCount = 0;

            try {
                msg = bnh0011_service.doExecService(new HashMap<String, String>());
                jobRlt = "S";
            } catch (Exception e) {
            	...
            	if( telNums != null ) {
	            	for (String telNum : telNums) {
	                    try {
	    	                ...
	    	                smsMap.put("CONTENTS", "[전자구매Batch] IT-Portal 계약서전송(BNH0011) I/F시 오류가 발생");
                ...
                try {
                    ...
                    logData.put("JOB_ID", "BNH0011");
                    logData.put("JOB_NM", "IT Portal 계약서 전송");
                    logData.put("JOB_KEY", "");
