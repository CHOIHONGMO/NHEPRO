...
<e:ui>
	<script type="text/javascript">
		...
		var grid;
		var baseUrl  = "/nhepro/CCDU/CCDU0020";
		var multiYN  = ${param.multiYN == "Y" ? true : false};
		var readOnlyFlag   = ${param.READONLY=="Y" ? true : false};	// 전체 ReadOnly
		var custCdReadOnly = ${(param.CUST_READONLY=="Y" || EVF.isEmpty(param.CUST_READONLY)) ? true : false};	// 고객사 ReadOnly
		...
		function init() {
			...
			if (readOnlyFlag) {
                EVF.C("CORP_TYPE").setReadOnly(readOnlyFlag);
                EVF.C("CUST_CD").setReadOnly(custCdReadOnly);
            }
			else {
            	if(relatYn == "0") {
		...
		function doSearch() {
			...
			store.setGrid([grid]);
			store.load(baseUrl + "/ccdu0020_doSearch.so", function () {
		...
		function doChoose() {
			...
			var selectedData;
			if(multiYN) {
				selectedData = grid.getSelRowValue();
        ...
        function getCust() {
        	...
        	if(changeCorpType != '2'){
	            ...
	            store.load('/nhepro/CCDU/CCDU0010/ccdu0010_getCust.so?RELAT_YN='+relat_yn+'&CORP_TYPE='+corp_type, function() {
	...
	<e:window id="CCDU0020" onReady="init" initData="${initData}" title="${fullScreenName }" breadCrumbs="${breadCrumb }">

		<e:searchPanel id="form" title="${msg.M9999}" columnCount="2" labelWidth="${labelWidth}" useTitleBar="false" onEnter="doSearch">
			<e:inputHidden id="OGDP_FLAG" name="OGDP_FLAG" value="${empty form.OGDP_FLAG ? param.ogdpFlag : form.OGDP_FLAG}"/>
			<e:inputHidden id="CTRL_CD" name="CTRL_CD" value="${empty form.CTRL_CD ? param.CTRL_CD : form.CTRL_CD}" />
			<e:inputHidden id="DEL_FLAG" name="DEL_FLAG" value="${empty form.DEL_FLAG ? param.DEL_FLAG : form.DEL_FLAG}" />
			<e:row>
				...
				<e:field>
					<e:select id="CUST_CD" name="CUST_CD" value="" options="${custCdOptions}" width="${form_CUST_CD_W}" disabled="${form_CUST_CD_D}" readOnly="${form_CUST_CD_RO}" required="${form_CUST_CD_R}" placeHolder="" maskType="${form_CUST_CD_MT}" usePlaceHolder="false"/>
				</e:field>
			</e:row>
			<e:row>
			...
			<e:row>
				...
				<e:field>
					<e:select id="BRANCH_FLAG" name="BRANCH_FLAG" value="" options="${branchFlagOptions}" width="${form_BRANCH_FLAG_W}" disabled="${form_BRANCH_FLAG_D}" readOnly="${form_BRANCH_FLAG_RO}" required="${form_BRANCH_FLAG_R}" placeHolder="" maskType="${form_BRANCH_FLAG_MT}" />
				</e:field>
			</e:row>
		</e:searchPanel>
