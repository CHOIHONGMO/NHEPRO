...
//                        }
                ...
                gridStreet.cellClickEvent(function (rowIdx, colIdx, value, iRow, iCol) {
                    var selectedData = gridStreet.getRowValue(rowIdx);
                ...
                gridDistrict.cellClickEvent(function (rowIdx, colIdx, value, iRow, iCol) {
                    var selectedData = gridDistrict.getRowValue(rowIdx);
                    if ('${param.modalYn}' == 'true') {
                        parent['${param.callBackFunction}'](selectedData);
                        EVF.closeWindow();
                    } else {
        ...
        function doSearchByStreet() {
			...
			//if (streetTap) {
				if (EVF.V("streetName") == '') {
                    EVF.alert('${BADV_020_001}' + "${form_captionStreet_N }");
	                return;
	            }
	        	var store = new EVF.Store();
	        	store.setGrid([gridStreet]);
	            store.load(baseUrl + 'zipCode/doSearchByStreet.so', function() {
		...
		function doSearchByDistrict() {
			...
			//if (districtTap) {
				...
				if (EVF.V("districtName") == '') {
                    EVF.alert('${BADV_020_001}' + "${form_captionDistrist_N }");
	                return;
	            }
	            ...
	            store.load(baseUrl + 'zipCode/doSearchByDistrict.so', function() {
    ...
    <e:window id="BADV_020" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
