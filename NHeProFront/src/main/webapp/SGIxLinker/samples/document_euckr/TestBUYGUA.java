...
public class TestBUYGUA {
	...
	public static String composeXML(String templatePath, String docCode)
	{
		...
		String	head_mesg_type  ="BUYGUA";			/*[필수]문서코드	*/
		String	head_mesg_name  ="이행상품보증서";		/*[필수]문서명	*/
		...
		String	head_titl_name  ="이행상품보증서";		/*[필수]문서개요	*/
		...
		String cont_penl_amnt ="1000";				/*[선택]계약금액 */
		...
		try{
			...
			if(		
				...
				&&  dataToXml.setData("cont_penl_amnt", cont_penl_amnt)
				...
				)
			{
				...
				FileWriteUtil.genFileCreate( "D:/KICA_SGIxLinker/samples/document/sampleBUYGUA.xml", xmlDoc);
			}else{
				System.out.println(dataToXml.getErrorMsg());
			}
		}catch(Exception _e){
	...
	{					
		try{
			...
			String bond_hold_name = xmlToData.getData("bond_hold_name");/*[선택]채권자/채권자명 */
			  /* End of 보증내용 */		
			  /* Begin of 보증계약(주) */   
			String cont_name_text = xmlToData.getData("cont_name_text");/*[선택]계약건명 */
			String cont_penl_amnt = xmlToData.getData("cont_penl_amnt");/*[선택]계약금액 */
			String cont_curc_code = xmlToData.getData("cont_curc_code");/*[선택]계약금액/통화코드	*/
			String cont_numb_text = xmlToData.getData("cont_numb_text");/*[선택]계약번호	 */
			String cont_main_date = xmlToData.getData("cont_main_date");/*[선택]계약일자  */
			String cont_begn_date = xmlToData.getData("cont_begn_date");/*[선택]계약기간/계약시작일   */  
			String cont_fnsh_date = xmlToData.getData("cont_fnsh_date");/*[선택]계약기간/계약만료일   */  
			  /* End of 보증계약(주) */          
			  /* Begin of 특기사항 */            
			...
			System.out.println("cont_penl_amnt=" + cont_penl_amnt);
