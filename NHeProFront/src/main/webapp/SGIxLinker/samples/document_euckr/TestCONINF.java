	...
	public static String composeXML(String templatePath, String docCode)
	{
		...
		try{
			...
			if(		
				...
				//&&  dataToXml.setData("mcre_orga_name", mcre_orga_name)
/*
				String data = null;
				for(int i = 0 ; i < 2 ; i++){
...
*/
				...
				)			
			{
				...
				FileWriteUtil.genFileCreate( "D:/2.KICA_Solution/SG_SGIxLinker/samples/document/sampleCONINF.xml", xmlDoc);
			}else{
				System.out.println(dataToXml.getErrorMsg());
			}
		}catch(Exception _e){
	...
	{	
		try{
			...
			String cont_unit_divs = xmlToData.getData("cont_unit_divs");
			/* End of 계약정보 */	                          
			/* Begin of 채권자 정보 */		            
			...
			/* End of 수요자 정보 */
			 ...
			 System.out.println("cont_unit_divs=" + cont_unit_divs);	
