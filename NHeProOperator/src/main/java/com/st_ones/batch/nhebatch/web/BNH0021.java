...
@Service
public class BNH0021 extends EverJob {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (super.isAllowedToRunBatch(context)) {
            ...
            bnh0021_service = SpringContextUtil.getBean(BNH0021_Service.class);

            String msg = "";
            int totalCount = 0;

            try {
                msg = bnh0021_service.doExecService(new HashMap<String, String>());
                jobRlt = "S";
            } catch (Exception e) {
            	...
            	if( telNums != null ) {
	            	for (String telNum : telNums) {
	                    try {
	    	                ...
	    	                smsMap.put("CONTENTS", "[전자구매Batch] SMS계약번호 배치(BNH0021) I/F시 오류가 발생");
                ...
                try {
                    ...
                    logData.put("JOB_ID", "BNH0021");
                    logData.put("JOB_NM", "SMS계약번호");
                    logData.put("JOB_KEY", "");
