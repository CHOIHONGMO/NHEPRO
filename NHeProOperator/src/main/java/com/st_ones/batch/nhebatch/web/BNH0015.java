...
import com.st_ones.batch.nhebatch.service.BNH0015_Service;
...
@Service
public class BNH0015 extends EverJob {
    @Autowired private BNH0001_Service bnh0001_service;
    @Autowired private BNH0015_Service bnh0015_service;
    @Autowired private EverSmsService everSmsService;
    
    /**
     * 화면명 : Batch 실행
     * 처리내용 : 임찰마감 및 입찰서 제출 기한 사전안내
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (super.isAllowedToRunBatch(context)) {
            ...
            bnh0015_service = SpringContextUtil.getBean(BNH0015_Service.class);

            String msg = "";
            int totalCount = 0;

            try {
                msg = bnh0015_service.doExecService(new HashMap<String, String>());
                jobRlt = "S";
            } catch (Exception e) {
            	...
            	if( telNums != null ) {
	            	for (String telNum : telNums) {
	                    try {
	    	                ...
	    	                smsMap.put("CONTENTS", "[전자구매Batch] 임찰마감 및 제출기한 사전안내(BNH0015) I/F시 오류가 발생");
                ...
                try {
                    ...
                    logData.put("JOB_ID", "BNH0015");
                    logData.put("JOB_NM", "입찰 제출기한 종료 및 마감처리 사전안내");
                    logData.put("JOB_KEY", "");
