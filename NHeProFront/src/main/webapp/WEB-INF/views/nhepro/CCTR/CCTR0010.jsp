...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
	<script>
		...
		var grid;
		var baseUrl = "/nhepro/CCTR/CCTR0010";
		...
		function init() {
			...
			grid.cellClickEvent(function(rowIdx, colIdx) {
				switch (colIdx) {
					case "FORM_NM":
						...
						everPopup.openPopupByScreenId('CCTI0011', 1200, 700, param);
						break;
				}
			});
			...
			EVF.C('BUYER_NM').setValue('${ses.companyNm}');
		...
		function doSearch() {
			...
			store.setGrid([grid]);
			store.load(baseUrl + "/cctr0010_doSearch.so", function () {
		...
		function doDelete() {
			...
			EVF.confirm("${msg.M0013 }", function () {
				...
				store.load(baseUrl + "/cctr0010_doDelete.so", function () {
		...
		function doUpdate() {
			...
			for(var i in rowIdx) {
				...
				if( (signStatus == "P" || signStatus == "E" || formUseCnt > 0) && useYn == "0" ) {
					msg = "${CCTR0010_0005}";
				}
			}
			...
			EVF.confirm(msg, function () {
				...
				store.setGrid([grid]);
				store.getGridData(grid, "sel");
				store.load(baseUrl + "/cctr0010_doUpdate.so", function () {
		...
		function doCopy() {
			...
			EVF.confirm("${CCTR0010_0002 }", function () {
				var store = new EVF.Store();
				store.setGrid([grid]);
				store.getGridData(grid, "sel");
				store.load(baseUrl + "/cctr0010_doCopy.so", function () {
					EVF.alert("${CCTR0010_0003 }", function () {
						doSearch();
					});
				});
			});
	...
	<e:window id="CCTR0010" initData="${initData}" onReady="init" title="${fullScreenName }" breadCrumbs="${breadCrumb }">
