...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    ...
    <script type="text/javascript">
        ...
        function init() {
            ...
            if(PROGRESS_CD == "") {
                ...
                EVF.C("VOC_OBJ").removeOption("USNC");
				
                if(vocType == "ST") {
                    EVF.C("VOC_TYPE").setReadOnly(true);
                    EVF.V("VOC_TYPE", vocType);
                    EVF.C("VOC_OBJ").setReadOnly(true);
                    EVF.V("VOC_OBJ", "USNI");
                }
            }
        ...
        function onChangePROGRESS_CD() {
            ...
            if(value != "" && Number(value) < Number(orgValue)) {
                EVF.alert("${CETR0041_001}", function () {
        ...
        function callbackVOC_OBJ_SUP_NM(data) {
            EVF.V("VOC_OBJ_SUP_CD" , data.VENDOR_CD);
            EVF.V("VOC_OBJ_SUP_NM" , data.VENDOR_NM);
        }
        ...
        function doSaveReq() {
            ...
            EVF.confirm("${msg.M0053}", function () {
                store.doFileUpload(function() {
                    store.load(baseUrl + "cetr0041_doSaveReq.so", function() {
        ...
        function doSave() {
            ...
            EVF.confirm("${msg.M0021}", function () {
                store.doFileUpload(function() {
                    store.load(baseUrl + "cetr0041_doSave.so", function() {
                        ...
                        EVF.alert(this.getResponseMessage(), function() {
                            ...
                            if(PROGRESS_CD == "500") {
                                ...
                                location.href = baseUrl + 'CETR0041/view.so?' + $.param(param);
                            }
                        });
                    });
                });
        ...
        function doDelete() {
            EVF.confirm("${msg.M0013 }", function () {
                var store = new EVF.Store();
                store.load(baseUrl + "cetr0041_doDelete.so", function() {
                    EVF.alert(this.getResponseMessage(), function() {
                        if(opener) {
                            opener.doSearch();
                        }
    ...
    <e:window id="CETR0041" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
        ...
        <e:inputHidden id="DS_USER_ID" name="DS_USER_ID" value="${formData.DS_USER_ID}"/>
        ...
        <e:searchPanel id="form1" title="${form_CAPTION_N}" labelWidth="120px" labelAlign="${labelAlign}" columnCount="2" useTitleBar="false" onEnter="doSearch">
            ...
            <e:row>
				...
				<e:field>
					<e:search id="VOC_OBJ_DEPT_NM" name="VOC_OBJ_DEPT_NM" value="${formData.VOC_OBJ_DEPT_NM}" width="${form_VOC_OBJ_DEPT_NM_W}" maxLength="${form_VOC_OBJ_DEPT_NM_M}" onIconClick="" disabled="${form_VOC_OBJ_DEPT_NM_D}" readOnly="${form_VOC_OBJ_DEPT_NM_RO}" required="${form_VOC_OBJ_DEPT_NM_R}" maskType="${form_VOC_OBJ_DEPT_NM_MT}" />
				</e:field>
            </e:row>
            <e:row>
