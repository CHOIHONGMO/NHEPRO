package com.st_ones.nhepro.MOBILE_NHPT.service;
...
import com.st_ones.nhepro.MOBILE_NHPT.MobilePT_Mapper;
...
/**
 * The type MobilePT_Service
 */
@Service(value = "MobilePT_Service")
public class MobilePT_Service extends BaseService {

	@Autowired private MessageService msg;

	@Autowired private MobilePT_Mapper mobile_mapper;
	...
	public void certified_update(Map<String, String> param) throws Exception {
        ...
        try {
            ...
            String subject  = "[전자근로계약시스템] 본인확인";
            String contents = "[전자근로계약시스템] 본인확인 인증번호는 "+ param.get("CERTIFIED_NUMBER") + " 입니다. 정확히 입력해 주세요.";
            ...
            //2022.11.17 sendSms => sendSmsNhe로 변경
	...
	public void doSave(Map<String, String> param) throws Exception {
		...
		if("E".equals(param.get("PROGRESS_CD"))) {
			// PW 2회 중복체크
			List<Map<String, Object>> uspwList = mobile_mapper.MPTIDPW_040_doSearchUSPW(param);
		...
		if(!param.get("PASSWORD").equals("")) {
			mobile_mapper.MPTIDPW_010_InsertPW(param);
		}
		...
		if(!"E".equals(param.get("PROGRESS_CD"))) {
            ...
            String subject  = "[전자근로계약시스템] 개인근로자 회원 가입 승인 요청 (이름 : " + param.get("USER_NM") + ")";
	...
	public List<Map<String, Object>> MPTAGG_030_doSearch(Map<String, String> param) throws Exception{
		return mobile_mapper.MPTAGG_030_doSearch(param);
	}

	public Map<String, String> MPTIDPW_010_doSearch(Map<String, String> param) throws Exception {
		return mobile_mapper.MPTIDPW_010_doSearch(param);
	}

	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void MPTIDPW_010_Update(Map<String, String> userInfo) throws Exception {

		// SMS 전송
		String subject  = "[전자근로계약시스템] 사용자 비밀번호가 초기화 되었습니다.";
		String contentsText = "";
		if( !"".equals(EverString.nullToEmptyString(userInfo.get("CELL_NUM"))) ){
			try {
				Map<String, String> smsMap = new HashMap<>();

				contentsText = "[전자근로계약시스템]<BR>" +
						"<BR>" +
				...
				//smsMap.put("RECV_USER_NM", userInfo.get("USER_NM"));
				//smsMap.put("RECV_TEL_NUM", userInfo.get("CELL_NUM"));
				smsMap.put("EPRO_PS_DSC", "1");			// 1  : 구매
				smsMap.put("EPRO_WRS_DS", "52");		// epro_wrs_ds [상품코드] - 10 : RFI, 20 : 입찰, 30 : 일반입찰계약, 40 : 일반수의계약, 50 : BtoC계약, 60 : 위임계약, 70 : 문서보관, 80 : SMS, 90 : LMS, 100 : 문서생성
	            smsMap.put("EPRO_RATE_DSC", "01");		// 01 : 최초
				smsMap.put("SUBJECT", subject);
		...
		if( !"".equals(EverString.nullToEmptyString(userInfo.get("EMAIL")))) {
			try {
				...
				System.out.println("====이메일로직 체크 "+userInfo.get("EMAIL")+"===========" );
				mailData.put("SEND_USER_ID", "SYSTEM");
				mailData.put("SEND_USER_NM", PropertiesManager.getString("eversrm.system.mailSenderName"));
				mailData.put("RECV_USER_ID", userInfo.get("USER_ID"));
				mailData.put("DIRECT_TARGET", userInfo.get("EMAIL"));
				mailData.put("DIRECT_USER_NM", userInfo.get("USER_NM"));
		...
		mobile_mapper.MPTIDPW_040_resetPassword(userInfo);
	}

	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public Boolean MPTIDPW_040_resetPassword(Map<String, String> param) throws Exception {
		// PW 2회 중복체크
		List<Map<String, Object>> uspwList = mobile_mapper.MPTIDPW_040_doSearchUSPW(param);
		...
		mobile_mapper.MPTIDPW_040_resetPassword(param);
		
		return pwFlag;
	}

	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void MPTIDPW_010_InsertPW(Map<String, String> userInfo) throws Exception {
		mobile_mapper.MPTIDPW_010_InsertPW(userInfo);
	}
	...
	/*부서식 가져오기*/
	public List<Map<String, Object>> mobileHome_subFormList(Map<String, String> param) throws Exception {

		List<Map<String, Object>> contractList = mobile_mapper.mobileHome_subFormList(param);
		return contractList;
	}
	...
	public void mobile_doContract(Map<String, String> param) throws Exception {
        ...
        paymentMap.put("EPRO_WRS_DS", "52");		// epro_wrs_ds [상품코드] - 10 : RFI, 20 : 입찰, 30 : 일반입찰계약, 40 : 일반수의계약, 50 : BtoC계약, 60 : 위임계약, 70 : 문서보관, 80 : SMS, 90 : LMS, 100 : 문서생성
        paymentMap.put("EPRO_RATE_DSC","01"); 	// epro_rate_dsc [단가코드] - 01 : 최초, 02 : 재입찰/재계약/재요청
        paymentMap.put("CONT_TBL_ID", "STOCTCCT");	// 업무 Table명
		...
		HashMap<String, String> fileSaveInfo = fileAttachController.getFileSaveInfo(
				"TC", param.get("filePath"), "계약서_" + param.get("USER_NM") + "_" + EverDate.getDate2() + ".pdf",
