var JS_Crypto_AuthKey = "";
var JS_Crypto_API = (function() {
    ...
    function t(N, R, K, S) {
        ...
        try {
            var L = magicjs.hmac.create(N);
            L.init(magicjs.base64.decode(R));
            L.update(K);
            var O = L.generate();
            var P = magicjs.base64.encode(O);
            S(0, {
                Base64Result: P
            })
        } catch (Q) {
            S(ML4WebLog.getErrCode("Crypto_JS_genHmac"), {
                errCode: 888,
                errMsg: Q.message
            })
        }
    ...
    function v(O, R, N, P, S) {
        ML4WebLog.log("verifyHmac by javascript");
        try {
            var L = magicjs.hmac.create(O);
            L.init(magicjs.base64.decode(R));
            L.update(N);
            var K = L.verify(magicjs.base64.decode(P));
            S(0, {
                result: K
            })
        } catch (Q) {
            S(ML4WebLog.getErrCode("Crypto_JS_verifyHmac"), {
                errCode: 888,
                errMsg: Q.message
            })
        }
    ...
    function x(L, S, K, N, T) {
        ML4WebLog.log("encrypt by javascript");
        try {
            var P = magicjs.cipher.create(true, L, magicjs.base64.decode(S));
            P.init(magicjs.base64.decode(K));
            P.update(N);
            P.finish();
            var R = magicjs.base64.encode(P.output);
            var O = magicjs.hex.encode(P.output);
            ...
            if (typeof(T) == "undefined") {
                ML4WebLog.log("- encrypt Error = " + Q.message);
                return {
                    errCode: ML4WebLog.getErrCode("Crypto_JS_encrypt"),
                    errMsg: Q.message
                }
            } else {
                T(ML4WebLog.getErrCode("Crypto_JS_encrypt"), {
                    errCode: 888,
                    errMsg: Q.message
                })
            }
    ...
    function f(O, S, L, N, R) {
        ML4WebLog.log("decrypt by javascript");
        try {
            var K = magicjs.cipher.create(false, O, magicjs.base64.decode(S));
            K.init(magicjs.base64.decode(L));
            K.update(magicjs.base64.decode(N));
            K.finish();
            var Q = K.output.data;
            R(0, {
                stringResult: Q
            ...
            R(ML4WebLog.getErrCode("Crypto_JS_decrypt"), {
                errCode: 888,
                errMsg: P.message
            })
        }
    ...
    function H(N, S, L, U, R, T) {
        ML4WebLog.log("sign by javascript");
        try {
            var ah = "";
            if (L != null) {
                L = ML4WebApi.ml4web_crypto_api.SD_api(L);
                ah = magicjs.pkcs5.decrypt(S, L)
            } else {
                if (typeof(S) == "string") {
                    ah = magicjs.priKey.create(S)
                } else {
                    ah = S
                }
            }
            ...
            if (R != null || R != undefined) {
                var ab = "OPT_NONE";
                var ad = "OPT_USE_CONTNET_INFO";
                var K = "OPT_NO_CONTENT";
                var V = "OPT_SIGNKOREA_FORMAT";
                var W = "OPT_HASHED_CONTENT";
                var ag = "rsa15";
                var X = "rsa20";
                var aj = "RSASSA-PKCS1-V1_5";
                var af = "RSA-PSS";
                var Q = "RSA-OAEP";
                if (R.ds_pki_sign != undefined) {
                    var aa = 0;
                    var ae = 0;
                    for (var Z = 0; Z < R.ds_pki_sign.length; Z++) {
                        switch (R.ds_pki_sign[Z]) {
                            case ab:
                                ae = magicjs.pkcs7.signedData.format.none;
                                break;
                            case ad:
                                ae = magicjs.pkcs7.signedData.format.useContentInfo;
                                break;
                            case K:
                                ae = magicjs.pkcs7.signedData.format.noContent;
                                break;
                            case V:
                                ae = magicjs.pkcs7.signedData.format.signGateFormat;
                                break;
                            case W:
                                ae = magicjs.pkcs7.signedData.format.hashedContent;
                                break
                        }
                        if (Z == 0) {
                            aa = ae
                ...
                if (R.ds_pki_rsa != undefined) {
                    if (R.ds_pki_rsa == X) {
                        ac.scheme = af;
                        if (R.ds_pki_hash != undefined) {
                            ac.md = R.ds_pki_hash
                        }
                    } else {
                        ac.scheme = aj
            ...
            ML4WebLog.log("sign by javascript : Option " + JSON.stringify(ac));
            var O = null;
            if (R.ds_pki_signdata != "") {
                O = magicjs.pkcs7.signedData.create(R.ds_pki_signdata)
            } else {
                O = magicjs.pkcs7.signedData.create()
            }
            if (R.ds_msg_decode == "true") {
                O.content = magicjs.base64.decode(U)
            } else {
                if (R.ds_msg_decode == "hash") {
                    O.content = magicjs.hex.decode(U)
                } else {
                    O.content = U
                }
            }
            var P = null;
            if (R.ds_pki_signdata != "" && R.ds_pki_signData !== undefined) {
                O.addSign(Y, ah, ac);
                P = magicjs.base64.encode(O.toDer().data)
            } else {
                O.sign(Y, ah, ac);
                P = magicjs.base64.encode(O.toDer().data)
            }
            ...
            T(ML4WebLog.getErrCode("Crypto_JS_sign"), {
                errCode: 888,
                errMsg: ai.message
            })
        }
    ...
    function w(L, P, K, S, O, Q) {
        ML4WebLog.log("signature by javascript");
        try {
            var aa = "";
            if (K != null) {
                K = ML4WebApi.ml4web_crypto_api.SD_api(K);
                aa = magicjs.pkcs5.decrypt(P, K)
            } else {
                if (typeof(P) == "string") {
                    aa = magicjs.priKey.create(P)
                } else {
                    aa = P
                }
            }
            ...
            var W = {};
            if (typeof(L) == "string") {
                U = magicjs.x509Cert.create(L)
            } else {
                U = L
            }
            if (aa.algorithm == "kcdsa") {
                W.pubKey = U.pubKey;
                if (O.ds_pki_hash != undefined) {
                    W.md = O.ds_pki_hash
                }
            } else {
                if (O != null || O != undefined) {
                    var Z = "rsa15";
                    var T = "rsa20";
                    var ad = "RSASSA-PKCS1-V1_5";
                    var Y = "RSA-PSS";
                    var R = "RSA-OAEP";
                    if (O.ds_pki_sign != undefined) {
                        var V = 0;
                        var X = 0
                    }
                    if (O.ds_pki_rsa != undefined) {
                        if (O.ds_pki_rsa == T) {
                            W.scheme = Y;
                            if (O.ds_pki_hash != undefined) {
                                W.md = O.ds_pki_hash
            ...
            if (S === null || S === "") {
                var ac = U.subject;
                S = ML4WebApi.makeReverseDN(ac)
            }
            var N = magicjs.base64.encode(aa.generateSignature(S, W.md, W));
            Q(0, {
                stringResult: N
            })
        } catch (ab) {
            Q(ML4WebLog.getErrCode("Crypto_JS_sign"), {
                errCode: 888,
                errMsg: ab.message
            })
        }
    ...
    function h(R, L, S, U, T) {
        ML4WebLog.log("envelopedData by javascript");
        try {
            var P = magicjs.x509Cert.create(R);
            var O = magicjs.pkcs7.envelopedData.create();
            O.content = L;
            var K = null;
            if (U == null) {
                K = {
            ...
            O.addRecipient(P, K);
            O.encrypt(S);
            var N = magicjs.base64.encode(O.toDer().data);
            T(0, {
                stringResult: N
            })
        } catch (Q) {
            T(ML4WebLog.getErrCode("Crypto_JS_envelopedData"), {
                errCode: 888,
                errMsg: Q.message
            })
        }
    ...
    function n(V, ag, S, ae, ab, L, R, Q) {
        ML4WebLog.log("signedEnvelopedData by javascript");
        try {
            var O = magicjs.x509Cert.create(V);
            var N = magicjs.x509Cert.create(ag);
            var ai = magicjs.pkcs5.decrypt(S, ae);
            var Y = {};
            var X = {};
            var W = "OPT_NONE";
            var Z = "OPT_USE_CONTNET_INFO";
            var K = "OPT_NO_CONTENT";
            var T = "OPT_SIGNKOREA_FORMAT";
            var ad = "rsa15";
            var U = "rsa20";
            var ah = "RSASSA-PKCS1-V1_5";
            var ac = "RSA-PSS";
            var P = "RSA-OAEP";
            Y.scheme = ah;
            X.scheme = ah;
            ...
            var aa = magicjs.pkcs7.signedAndEnvData.create();
            aa.content = ab;
            aa.addRecipient(O, Y);
            aa.signAndEnv(N, ai, L, X);
            Q(0, {
                stringResult: magicjs.base64.encode(aa.toPem())
            })
        } catch (af) {
            Q(ML4WebLog.getErrCode("Crypto_JS_signedEnvelopedData"), {
                errCode: 888,
                errMsg: af.message
            })
        }
    ...
    function g(S, K, N, Q, R) {
        ML4WebLog.log("verifyVID by javascript");
        try {
            var L = magicjs.pkcs5.decrypt(K, N);
            var O = magicjs.x509Cert.create(S);
            var T = O.verifyVID(L.getRandomNum(), Q);
            R(0, {
                result: T
            })
        } catch (P) {
            R(ML4WebLog.getErrCode("Crypto_JS_verifyVID"), {
                errCode: 888,
                errMsg: P.message
            })
        }
    ...
    function z(L, Q) {
        ...
        try {
            var O = magicjs.generateRandomBytes(L);
            var K = magicjs.base64.encode(O);
            var N = magicjs.hex.encode(O);
            ...
            if (typeof(Q) == "undefined") {
                ML4WebLog.log("- genHash Error = " + P.message);
                return {
                    errCode: ML4WebLog.getErrCode("Crypto_JS_generateRandom"),
                    errMsg: P.message
                }
            } else {
                Q(ML4WebLog.getErrCode("Crypto_JS_generateRandom"), {
                    errCode: 888,
                    errMsg: P.message
                })
            }
    ...
    function I(L, O, Q) {
        ...
        try {
            var K = magicjs.generateKeyPair(L, {
                bits: O
            });
            var N = {};
            N.ds_pki_pubkey = magicjs.base64.encode(K.publicKey.toDer().data);
            N.ds_pki_prikey = magicjs.base64.encode(K.privateKey.toDer().data);
            Q(0, {
                result: N
            })
        } catch (P) {
            Q(ML4WebLog.getErrCode("Crypto_JS_genKeypair"), {
                errCode: 888,
                errMsg: P.message
            })
        }
    ...
    function F(L, K, S) {
        ...
        try {
            var P = magicjs.md.create(L);
            P.update(K);
            var R = P.digest();
            var O = magicjs.base64.encode(R);
            ...
            if (typeof(S) == "undefined") {
                ...
                S(ML4WebLog.getErrCode("Crypto_JS_genHash"), {
                    errCode: 888,
                    errMsg: Q.message
                })
            }
    ...
    function k(K, N, O, T) {
        ...
        try {
            var S = magicjs.md.create(K);
            var Q = N;
            while (O--) {
                Q = S.digest(Q)
            }
            var R = magicjs.base64.encode(Q);
            var L = magicjs.hex.encode(Q);
            T(0, {
                hash: R,
                resulthex: L
            })
    ...
    function s(O, L, Q) {
        ...
        L = ML4WebApi.ml4web_crypto_api.SD_api(L);
        try {
            var K = magicjs.pkcs5.decrypt(O, L);
            var P = magicjs.base64.encode(K.toDer().data);
            if (typeof(Q) == "undefined") {
                return {
                    errCode: 0,
                    Base64String: P,
            ...
            if (typeof(Q) == "undefined") {
                ML4WebLog.log("- prikeyDecrypt Error = " + N.message);
                return {
                    errCode: ML4WebLog.getErrCode("Crypto_JS_prikeyDecrypt"),
                    errMsg: N.message
                }
            } else {
                Q(ML4WebLog.getErrCode("Crypto_JS_prikeyDecrypt"), {
                    errCode: 888,
                    errMsg: N.message
                })
            }
    ...
    function e(Q, L, O, R) {
        ...
        try {
            ...
            var S = magicjs.priKey.create(Q);
            var P = magicjs.pkcs5.encrypt(S, L, T);
            var K = magicjs.base64.encode(P);
            R(0, {
                Base64String: K
            })
        } catch (N) {
    ...
    function q(L, O, Q) {
        ...
        try {
            O = ML4WebApi.ml4web_crypto_api.SD_api(O);
            var N = magicjs.pkcs5.decrypt(L, O);
            var K = magicjs.base64.encode(N.getRandomNum());
            Q(0, {
                result: K
            })
        } catch (P) {
            Q(ML4WebLog.getErrCode("Crypto_JS_getVIDRandom"), {
                errCode: 888,
                errMsg: P.message
            })
        }
    ...
    function E(V, K, N, S, U) {
        ML4WebLog.log("getVIDRandomHash by javascript");
        try {
            var P = magicjs.x509Cert.create(V);
            var L = magicjs.pkcs5.decrypt(K, N);
            var R = L.getRandomNum();
            if (R == null) {
                return null
            }
            var T = P.makeVID(R, S, 2);
            var Q = magicjs.base64.encode(T);
            U(0, {
                result: Q
            })
        } catch (O) {
            U(ML4WebLog.getErrCode("Crypto_JS_getVIDRandom"), {
                errCode: 888,
                errMsg: O.message
            })
        }
    ...
    function j(X, W, O, Y) {
        ...
        try {
            ...
            ML4WebLog.log("- asymEncrypt Option = " + JSON.stringify(K));
            var S = magicjs.x509Cert.create(X);
            var U = S.pubKey;
            Q = U.encrypt(W, K);
            var Z = magicjs.base64.encode(Q);
            ...
            if (typeof(Y) == "undefined") {
                ML4WebLog.log("- asymEncrypt Error = " + R.message);
                return ML4WebLog.getErrCode("Crypto_JS_asymEncrypt")
            } else {
                Y(ML4WebLog.getErrCode("Crypto_JS_asymEncrypt"), {
                    errCode: 888,
                    errMsg: R.message
                })
            }
    ...
    function c(Y, R, N, P, X) {
        ...
        try {
            ...
            ML4WebLog.log("- asymDecrypt Option = " + JSON.stringify(K));
            var W = magicjs.pkcs5.decrypt(Y, R);
            var Q = null;
            Q = W.decrypt(magicjs.base64.decode(N), K);
            X(0, {
                result: Q
            })
        } catch (T) {
            X(ML4WebLog.getErrCode("Crypto_JS_asymDecrypt"), {
                errCode: 888,
                errMsg: T.message
            })
        }
    ...
    function r(K, L, Q) {
        ...
        L = ML4WebApi.ml4web_crypto_api.SD_api(L);
        try {
            var P = magicjs.pkcs12.create(K);
            P.importPfx(L);
            var N = {};
            if (P.safeContents.length < 1) {
                Q(ML4WebLog.getErrCode("Crypto_JS_pfxImport"), {
                    errCode: 888,
                    errMsg: "pfx is empty"
                })
            }
            if (P.safeContents.length == 1) {
                b(P.safeContents[0].cert.toPem(), ["keyusage"], function(S, R) {
                    if (S == 0) {
                        if (R.result.keyusage.indexOf("digitalSignature") != -1) {
                            N.signcert = magicjs.base64.encode(P.safeContents[0].cert.toDer());
                            e(magicjs.base64.encode(P.safeContents[0].priKey.toDer()), L, null, function(U, T) {
                                if (U == "0") {
                                    N.signpri = T.Base64String
                                } else {
                                    Q(ML4WebLog.getErrCode("Crypto_JS_prikeyEncrypt"), {
                                        errCode: U,
                                        errMsg: T
                                    })
                                }
                    ...
                    if (S == 0) {
                        if (R.result.keyusage.indexOf("digitalSignature") != -1) {
                            ...
                            e(magicjs.base64.encode(P.safeContents[0].priKey.toDer()), L, null, function(U, T) {
                                if (U == "0") {
                                    N.signpri = T.Base64String;
                                    N.kmcert = magicjs.base64.encode(P.safeContents[1].cert.toDer());
                                    e(magicjs.base64.encode(P.safeContents[1].priKey.toDer()), L, null, function(W, V) {
                                        if (W == "0") {
                                            N.kmpri = V.Base64String;
                                            Q(0, {
                                                result: N
                                            })
                                        } else {
                            ...
                            N.signcert = magicjs.base64.encode(P.safeContents[1].cert.toDer());
                            e(magicjs.base64.encode(P.safeContents[1].priKey.toDer()), L, null, function(U, T) {
                                if (U == "0") {
                                    N.signpri = T.Base64String;
                                    N.kmcert = magicjs.base64.encode(P.safeContents[0].cert.toDer());
                                    e(magicjs.base64.encode(P.safeContents[0].priKey.toDer()), L, null, function(W, V) {
                                        if (W == "0") {
                                            N.kmpri = V.Base64String;
                                            Q(0, {
                                                result: N
            ...
            if (O.code == 184573952) {
                L = ML4WebApi.ml4web_crypto_api.HD_api(L);
                d(K, L, Q)
            } else {
                Q(ML4WebLog.getErrCode("Crypto_JS_pfxImport"), {
                    errCode: 888,
    ...
    function d(K, L, R) {
        ...
        try {
            ...
            if (Q.safeContents.length == 1) {
                b(Q.safeContents[0].cert.toPem(), ["keyusage"], function(T, S) {
                    if (T == 0) {
                        if (S.result.keyusage.indexOf("digitalSignature") != -1) {
                            N.signcert = magicjs.base64.encode(Q.safeContents[0].cert.toDer());
                            N.signpri = magicjs.base64.encode(Q.safeContents[0].priKey);
                            R(0, {
                                result: N
                            })
                        } else {
                ...
                b(Q.safeContents[0].cert.toPem(), ["keyusage"], function(T, S) {
                    if (T == 0) {
                        if (S.result.keyusage.indexOf("digitalSignature") != -1) {
                            ...
                            N.signpri = magicjs.base64.encode(Q.safeContents[0].priKey);
                            if (Q.safeContents.length > 1) {
                                N.kmcert = magicjs.base64.encode(Q.safeContents[1].cert.toDer());
                                N.kmpri = magicjs.base64.encode(Q.safeContents[1].priKey)
                            }
                            R(0, {
                                result: N
                            })
                        } else {
                            N.signcert = magicjs.base64.encode(Q.safeContents[1].cert.toDer());
                            N.signpri = magicjs.base64.encode(Q.safeContents[1].priKey);
                            N.kmcert = magicjs.base64.encode(Q.safeContents[0].cert.toDer());
                            N.kmpri = magicjs.base64.encode(Q.safeContents[0].priKey);
                            R(0, {
                                result: N
                            })
                        }
    ...
    function D(P, K, S) {
        ...
        K = ML4WebApi.ml4web_crypto_api.SD_api(K);
        try {
            var O = magicjs.pkcs12.create();
            var R = magicjs.x509Cert.create(P.signcert);
            var U = "";
            if (typeof(P.pkcs5decrypt) == "undefined" || P.pkcs5decrypt == true) {
                U = magicjs.pkcs5.decrypt(P.signpri, K)
            } else {
                U = P.signpri
            }
            O.safeContents.push({
            ...
            if (P.kmcert != null && P.kmcert != undefined && P.kmcert.length != 0) {
                var Q = magicjs.x509Cert.create(P.kmcert);
                var L = "";
                if (typeof(P.pkcs5decrypt) == "undefined" || P.pkcs5decrypt == true) {
                    L = magicjs.pkcs5.decrypt(P.kmpri, K)
                } else {
                    L = P.kmpri
                }
                O.safeContents.push({
            ...
            var T = magicjs.base64.encode(O.toDer());
            S(0, {
                result: T
            })
        } catch (N) {
            S(ML4WebLog.getErrCode("Crypto_JS_pfxExport"), {
                errCode: 888,
                errMsg: N.message
            })
        }
    ...
    function o(P, K, T, S) {
        ...
        try {
            var O = magicjs.pkcs12.create();
            var R = magicjs.x509Cert.create(P.signcert);
            var V = magicjs.pkcs5.decrypt(P.signpri, K);
            O.safeContents.push({
                cert: R,
                priKey: V
            });
            if (P.kmcert != null && P.kmcert != undefined && P.kmcert.length != 0) {
                var Q = magicjs.x509Cert.create(P.kmcert);
                var L = magicjs.pkcs5.decrypt(P.kmpri, K);
                O.safeContents.push({
                    cert: Q,
                    priKey: L
                })
            ...
            var U = magicjs.base64.encode(O.toDer());
            S(0, {
                result: U
            })
        } catch (N) {
            S(ML4WebLog.getErrCode("Crypto_JS_pfxChangePwExport"), {
                errCode: 888,
                errMsg: N.message
            })
        }
    ...
    function b(V, S, al) {
        ...
        try {
            ...
            var ar = "subjectname";
            var ai = "pubkey";
            var ah = "pubkeyalgorithm";
            var P = "keyusage";
            var at = "certpolicy";
            var O = "policyid";
            var ab = "policynotice";
            var am = "subjectaltname";
            var ad = "authkeyid";
            var aa = "subkeyid";
            var Q = "crldp";
            var ak = "aia";
            var aj = "realname";
            var N = magicjs.x509Cert.create(V);
            var X = {};
            var ay = {};
            var an = N.extensions.length;
            if (an > 0) {
                for (var ax = 0; ax < an; ax++) {
                    if (N.extensions[ax].aki != null) {
                        ay.aki = "KeyID=" + N.extensions[ax].aki.keyIdentifier.toHex() + ",\nCertificate Issuer:\n" + N.extensions[ax].aki.authorityCertIssuer + ",\nCertificate SerialNumber:\n" + N.extensions[ax].aki.authorityCertIssuer
                    } else {
                        if (N.extensions[ax].ski != null) {
                            ay.subkeyid = N.extensions[ax].ski.toHex()
                        } else {
                            if (N.extensions[ax].keyUsage != null) {
                                ay.keyusage = m(N.extensions[ax].keyUsage)
                            } else {
                                if (N.extensions[ax].certPolicies != null) {
                                    var aq = null;
                                    var ap = null;
                                    var ao = N.extensions[ax].certPolicies.length;
                                    for (var aw = 0; aw < ao; aw++) {
                                        ay.policyid = N.extensions[ax].certPolicies[aw].policyIdentifier;
                                        if (N.extensions[ax].certPolicies[aw].cps != null) {
                                            aq = N.extensions[ax].certPolicies[aw].cps
                                        }
                                        if (N.extensions[ax].certPolicies[aw].unotice != null && N.extensions[ax].certPolicies[aw].unotice.explicitText != null) {
                                            ap = N.extensions[ax].certPolicies[aw].unotice.explicitText
                                        }
                                    }
                                    ay.policynotice = ap;
                                    ay.certpolicy = "policyID = " + ay.policyid + ",\ncpsUri  = " + aq + ",\nUserNotice  = " + ap + ",\n"
                                } else {
                                    if (N.extensions[ax].subjectAltName != null || N.extensions[ax].issuertAltName != null) {
                                        var Z = null;
                                        if (N.extensions[ax].subjectAltName != null) {
                                            Z = N.extensions[ax].subjectAltName
                                        } else {
                                            Z = N.extensions[ax].issuertAltName
                                        }
                                        var T = Z.length;
                                        for (var aw = 0; aw < T; aw++) {
                                            if (Z[aw].otherName != null) {
                                                ay.subjectaltname = "-identifyData\n- realName= " + Z[aw].otherName.realName;
                                                ay.realname = Z[aw].otherName.realName;
                                                if (Z[aw].otherName.vid != null) {
                                                    ay.subjectaltname += "- vid (" + Z[aw].otherName.vid.hashAlg + ")= " + Z[aw].otherName.vid.value.toHex()
                                                }
                                            }
                                        }
                                    } else {
                                        if (N.extensions[ax].crlDPs != null) {
                                            var Y = "";
                                            var aB = N.extensions[ax].crlDPs.length;
                                            for (var aw = 0; aw < aB; aw++) {
                                                Y += N.extensions[ax].crlDPs[aw].distributionPoint[0].uri + " "
                                            }
                                            ay.crldp = Y
                                        } else {
                                            if (N.extensions[ax].aia != null) {
                                                var ae = "";
                                                if (N.extensions[ax].aia.ocsp != null) {
                                                    ae += N.extensions[ax].aia.ocsp.uri
                                                }
                                                ay.aia = ae
                                            }
                                        }
            ...
            for (var ax = 0; ax < au; ax++) {
                ...
                switch (S[ax]) {
                    case av:
                        ...
                        break;
                    case U:
                        X.serialnum = N.serialNum;
                        break;
                    case aA:
                        X.signaturealgorithm = N.signAlg.name;
                        break;
                    case W:
                        X.issuername = N.issuer;
                        break;
                    case ac:
                        var ag = N.validity.notBefore.getMonth() + 1;
                        X.startdate = N.validity.notBefore.getFullYear() + "-" + ag + "-" + N.validity.notBefore.getDate();
                        break;
                    case af:
                        var ag = N.validity.notAfter.getMonth() + 1;
                        X.enddate = N.validity.notAfter.getFullYear() + "-" + ag + "-" + N.validity.notAfter.getDate();
                        break;
                    case L:
                        var ag = N.validity.notBefore.getMonth() + 1;
                        X.startdatetime = N.validity.notBefore.getFullYear() + "-" + ag + "-" + N.validity.notBefore.getDate() + " " + N.validity.notBefore.getHours() + ":" + N.validity.notBefore.getMinutes() + ":" + N.validity.notBefore.getSeconds();
                    case K:
                        var ag = N.validity.notAfter.getMonth() + 1;
                        X.enddatetime = N.validity.notAfter.getFullYear() + "-" + ag + "-" + N.validity.notAfter.getDate() + " " + N.validity.notAfter.getHours() + ":" + N.validity.notAfter.getMinutes() + ":" + N.validity.notAfter.getSeconds();
                        break;
                    case ar:
                        X.subjectname = N.subject;
                        break;
                    case ai:
                        X.pubkey = N.pubKey.toDer().toHex();
                        break;
                    case ah:
                        X.pubkeyalgorithm = N.pubKey.algorithm + " (" + N.pubKey.keyLength + " bits)";
                        break;
                    case P:
                        X.keyusage = ay.keyusage;
                        break;
                    case at:
                        X.certpolicy = ay.certpolicy;
                        break;
                    case O:
                        X.policyid = ay.policyid;
                        break;
                    case am:
                        X.subjectaltname = ay.subjectaltname;
                        break;
                    case ad:
                        X.authkeyid = ay.authkeyid;
                        break;
                    case aa:
                        X.subkeyid = ay.subkeyid;
                        break;
                    case Q:
                        X.crldp = ay.crldp;
                        break;
                    case ak:
                        ...
                        break;
                    case aj:
                        X.realname = ay.realname;
                        break;
                    case ab:
                        X.policynotice = ay.policynotice;
                        break
                }
            }
            al(0, {
    ...
    function u(K, N, L, P, R) {
        ML4WebLog.log("prikeyChangePassword by javascript");
        try {
            var Q = magicjs.pkcs5.changePassword(K, N, L);
            R(0, {
                result: magicjs.base64.encode(Q)
            })
        } catch (O) {
            R(ML4WebLog.getErrCode("Crypto_JS_prikeyChangePassword"), {
                errCode: 888,
                errMsg: O.message
            })
        }
    ...
    function J(N, L, K, P, Q) {
        ML4WebLog.log("certChangePassword by javascript");
        try {
            if (N == null || $.isEmptyObject(N)) {
                ML4WebLog.log("invalid param : jsonCert");
                Q(100, $.i18n.prop("ER100") + "[jsonCert]");
                return
            } else {
                if (L == null || L == "") {
                    ML4WebLog.log("invalid param : sOldPassword");
                    Q(100, $.i18n.prop("ER100") + "[sOldPassword]");
                    return
                } else {
                    if (K == null || K == "") {
                        ML4WebLog.log("invalid param : sNewPassword");
                        Q(100, $.i18n.prop("ER100") + "[sNewPassword]");
                        return
                    } else {
                        if (typeof(N.signpri) == "undefined" || typeof(N.signpri) != "string") {
                            ML4WebLog.log("invalid param : jsonCert.signpri");
                            Q(100, $.i18n.prop("ER100") + "[jsonCert.signpri]");
                            return
                        }
                    }
                }
            ...
            L = ML4WebApi.ml4web_crypto_api.SD_api(L);
            K = ML4WebApi.ml4web_crypto_api.SD_api(K);
            N.signpri = magicjs.base64.encode(magicjs.pkcs5.changePassword(N.signpri, L, K));
            if (typeof(N.kmpri) != "undefined" && typeof(N.kmpri) == "string") {
                N.kmpri = magicjs.base64.encode(magicjs.pkcs5.changePassword(N.kmpri, L, K))
            }
            Q(0, {
                result: N
            })
        } catch (O) {
            Q(ML4WebLog.getErrCode("Crypto_JS_certChangePassword"), {
                errCode: 888,
                errMsg: O.message
            })
        }
    ...
    function a(P, O) {
        var T = magicjs.pkcs7.signedData.create();
        var L = "";
        var S = "";
        var Q = "";
        var R = "sha256";
        var N = O;
        var K;
        if (P == "signedAttribute") {
            if (N.signTime.indexOf("T") < 0) {
                N.signTime = N.signTime.replace(" ", "T")
            }
            K = new Date(N.signTime);
            browser = ML4WebApi.get_browser_info();
            browserName = browser.name;
            if ((browserName.toLowerCase().indexOf("safari") > -1)) {
                K = new Date(K.setTime(K - (9 * 60 * 60 * 1000)))
            }
            L = {
                signingTime: K
            };
            S = T.makeTBSData(magicjs.utf8.encode(N.plainText), R, L)
        } else {
            S = magicjs.utf8.encode(N.plainText)
        }
        Q = "3031300d060960864801650304020105000420" + l(S);
        ML4WebLog.log("digestInfo : " + Q);
        Q = magicjs.base64.encode(Q);
        ML4WebLog.log("base64 encoding digestInfo : " + Q);
        return Q
    }

    function l(N) {
        var K = magicjs.md.create("sha256");
        K.init();
        var L = K.digest(N);
        return L.toHex()
    }

    function C(Q, S, R, O, L) {
        var U = magicjs.x509Cert.create(Q);
        var X = "RSASSA-PKCS1-V1_5";
        var T = "sha256";
        ...
        if (S === "originHash") {
            K.format = magicjs.pkcs7.signedData.format.noSignedAttributes
        }
        W.compose(1, K, N, V, magicjs.base64.decode(O));
        return magicjs.base64.encode(W.toDer())
    }

    function A(L) {
        tbhdata = magicjs.utf8.encode(L);
        var K = p(tbhdata);
    ...
    return {
        ...
        asymDecrypt: c,
        pfxImport: r,
        pfxImport_withOption: d,
        pfxExport: D,
        pfxChangePwExport: o,
        getcertInfo: b,
        prikeyChangePassword: u,
        certChangePassword: J,
        signature: w,
        signedAttributes: a,
        pkcs7: C
    }
...
function completeFunc(j, f, e) {
    ...
    if (typeof d.signResult.b64SignedData != "undefined" && d.signResult.b64SignedData != "") {
            ...
            if (n instanceof Array) {
                for (var q = 0; q < n.length; q++) {
                    ...
                    if (l instanceof Array) {
                        c[q] = h.pkcs7(r, m, l[q], n[q], p)
                    } else {
                        c[q] = h.pkcs7(r, m, l, n[q], p)
                    }
                    o.push(c[q])
                }
            }
        ...
        h.getcertInfo(t, [], function(u, v) {
            if (u == 0) {
                g.certbag = {};
                g.certbag.signcert = t;
                g.certInfo = v.result;
                ML4WebApi.webConfig.getRandomfromPrivateKey = a;
...
var Storage_API_web = {
    ...
    ChangePassword: function(g, j, c, o) {
        ...
        try {
            ...
            if (b != null && l > 0) {
                for (var d = 0; d < l; d++) {
                    if (b[d].storageCertIdx == g.storageCertIdx) {
                        var k = b[d];
                        n.prikeyDecrypt(k.signpri, j, function(e, p) {
                            if (e == 0) {
                                n.prikeyEncrypt(p.Base64String, c, null, function(r, s) {
                                    if (r == 0) {
                                        if (ML4WebApi.getProperty("libType") == 0) {
                                            k.signpri = s
                                        } else {
                                            k.signpri = s.Base64String
                                        }
                                        if (k.kmpri) {
                                            n.prikeyDecrypt(k.kmpri, j, function(t, u) {
                                                if (t == 0) {
                                                    n.prikeyEncrypt(u.Base64String, c, null, function(x, v) {
                                                        if (x == 0) {
                                                            if (ML4WebApi.getProperty("libType") == 0) {
                                                                k.kmpri = v
                                                            } else {
                                                                k.kmpri = v.Base64String
                                                            }
                                                            var w = Storage_API_web.SaveCert;
                                                            w(k, c, g, function(z, y) {
                                                                if (z == 0) {
                                                                    o(0, {
                                                                        result: true
                                                                    })
                                                                } else {
                                                                    o(ML4WebLog.getErrCode("Storage_Web_ChangePassword"), {
                                                                        errCode: z,
                                                                        errMsg: y.errMsg
                                                                    })
                                                                }
                                            ...
                                            var q = JSON.stringify(g);
                                            Storage_API_web.SaveCert(k, c, g, function(u, t) {
                                                if (u == 0) {
                                                    o(0, {
                                                        result: true
                                                    })
...
var Storage_API_token = {
    SelectStorageInfo: function(b, h) {
        ...
        var a = ML4WebApi.getCsManager();
        try {
            a.callLocalServerAPI(d, c, function(j, n) {
                if (j == 0) {
                    var e = JSON.parse(n);
                    if (e.ResultCode == 0) {
                        var l = JSON.parse(e.ResultMessage);
                        var o = {};
                        if (e.ResultMessage.indexOf("USIM_0002") < 0) {
                            var m = {};
                            m.tokenname = "Mobile_SmartCert";
                            m.driver = "USIM_0002";
                            m.driverPath = "";
                            o.tokenOpt = [];
                            o.tokenOpt.push(m)
                        ...
                        if (l.tokenOpt.length > 0) {
                            if (o.tokenOpt == null) {
                                ...
                                for (var k = 0; k < l.tokenOpt.length; k++) {
                                    o.tokenOpt.push(l.tokenOpt[k])
                                }
                            }
                        }
                        h(0, o);
...
var Storage_API_smartcertnx = {
    ...
    Sign: function(g, l, r, h, f) {
        ...
        p = p.replace(/\,/g, "|");
        var u = new Object();
        u.signOrgView = ML4WebApi.getProperty("cs_smartcert_signorgview");
        ...
        var c = new Array();
        var b = new Array();
        if (u.plainText instanceof Array) {
            var e = u.plainText;
            for (var s = 0; s < e.length; s++) {
                j[s] = new Object();
                u.plainText = e[s];
                j[s].msg = k.signedAttributes(n, u);
                q.push(j[s]);
                c[s] = new Object();
                c[s].signOrgView = u.signOrgView;
                c[s].plainText = u.plainText;
                if (typeof(u.signTime) != "undefined") {
                    c[s].signTime = u.signTime
                }
                b.push(c[s])
            }
            a = JSON.stringify(q);
            ...
            j = new Object();
            j.msg = k.signedAttributes(n, u);
            a = JSON.stringify(j);
            msgIntegrity = JSON.stringify(u)
        }
        var h = "";
...
var Storage_API_filter = {
    ...
    selectfilteredCertList: function(l, s) {
        var n = [];
        for (var f = 0; f < l.length; f++) {
            ...
            if (filter_type != null && filter_type != "") {
                ...
                var o = [];
                for (var d = 0; d < 4; d++) {
                    o.push(e.substring(d, d + 1))
                }
                var p = l[f].issuername.split(",");
                $.each(p, function() {
                    var j = this.split("=");
                    if (j[0].toLowerCase() == "ou") {
                        ...
                        if (j[0].toLowerCase() == "o") {
                            if (j[1].toLowerCase() == "yessign" || j[1].toLowerCase() == "kica" || j[1].toLowerCase() == "crosscert" || j[1].toLowerCase() == "tradesign" || j[1].toLowerCase() == "signkorea" || j[1].toLowerCase() == "inipass") {
                                if (o[2] == "0") {
                                    a = true
                                } else {
                                    a = false
            ...
            if (!a && filter_use_type != null && filter_use_type != "") {
                var p = l[f].keyusage.split(",");
                var g = "";
                if (p.length == 2) {
                    g = "1"
                } else {
            ...
            if (!a && filter_expire) {
                var b = ML4WebUtil.isDateExpired(l[f].enddatetime);
                if (b) {
                    a = true
                } else {
                    a = false
            ...
            if (!a && filter_oid != null && filter_oid != "") {
                var c = l[f].policyid;
                var o = filter_oid.split(",");
                var h = o.length;
                for (var d = 0; d < h; d++) {
                    if (o[d] == c) {
                        a = false;
            ...
            if (!a && filter_subject_dn) {
                var c = l[f].policyid;
                var o = oid_name.split(",");
                var h = o.length;
                for (var d = 0; d < h; d++) {
                    if (o[d] == c) {
                        a = false
            ...
            if (!a && filter_sign_serial != null && filter_sign_serial != "") {
                var m = l[f].serialnum;
                if (filter_sign_serial == m) {
                    a = false
                } else {
                    a = true
                }
            }
            if (!a && filter_km_serial != null && filter_km_serial != "") {
                var m = l[f].serialnum;
                if (filter_km_serial == m) {
                    a = false
                } else {
                    a = true
                }
            }
            if (!a && oid_name != null && oid_name != "") {
                var c = l[f].policyid;
                var o = oid_name.split(",");
                var h = o.length;
                for (var d = 0; d < h; d++) {
            ...
            if (!a && issuer_name != null && issuer_name != "") {
                var p = l[f].issuername.split(",");
                var k = "";
                $.each(p, function() {
                    var j = this.split("=");
            ...
            if (!a && filter_path != null && filter_path != "") {
                if (typeof(l[f].certpath) != "undefined") {
                    var r = l[f].certpath;
                    r = r.toUpperCase();
                    for (var d = 0; d < filter_path.length; d++) {
                        if (r.indexOf(filter_path[d].toUpperCase()) > -1) {
                            a = true;
                            break
                        } else {
                            a = false
...
var CS_Manager_API = (function(p, m) {
    ...
    function r() {
        ...
        try {
            ...
            for (var y = 0; y < 20; y++) {
                A += x.charAt(Math.floor(Math.random() * x.length))
            }
            return {
                code: 0,
                data: A
...
var ML4WebApi = {
    ...
    ml4web_crypto_api: {
        ...
        init: function(blibType, resource_api, callback) {
            ML4WebLog.log("Crypto_API Init() called...");
            if (blibType == null || blibType == "undefined") {
                libType = resource_api.checkCS()
            } else {
                libType = blibType
            }
            if (libType == 0) {
                cryptObj = new C_Crypto_API(resource_api)
            } else {
                if (libType == 1) {
                    cryptObj = new JS_Crypto_API(resource_api)
                } else {
                    if (libType == 2) {
                        cryptObj = new JS_Crypto_Raw_API(resource_api)
                    }
                }
            }
            var magicjsUrl = "js/crypto/magicjs_1.2.7.2.min.js";
            $.cachedScript(magicjsUrl).done(function(script, status) {
                eval(script);
                cryptObj.init(callback)
            })
        },
        ...
        getEncryptedCert: function(b) {
            try {
                if (b !== null && typeof b !== "undefined" && b !== "") {
                    var a = magicjs.cipher.create(true, "SEED-CBC", magicjs.hex.decode("0b9f0bb6f0a89844aedecb9f2508c7e2"));
                    a.init(magicjs.hex.decode("69068374ba84d85b68eea844d2d2c187"));
                    a.update(b);
                    a.finish();
        ...
        getDecryptedCert: function(b) {
            try {
                if (b !== null && typeof b !== "undefined" && b !== "") {
                    var a = magicjs.cipher.create(false, "SEED-CBC", magicjs.hex.decode("0b9f0bb6f0a89844aedecb9f2508c7e2"));
                    a.init(magicjs.hex.decode("69068374ba84d85b68eea844d2d2c187"));
        ...
        incaDecrypt: function(d, g) {
            var b = d.substr(0, 32);
            var c = d.substr(32);
            var f = magicjs.base64.encode(magicjs.hex.decode("d4219a6be6b839e34e73e52c9b2213c3d254fed02637abce605a887e1c393dde"));
        ...
        pkcs7: function(g, b, f, a, d) {
            ...
            try {
                var c = cryptObj.pkcs7(g, b, f, a, d);
                return c
            } catch (h) {
                callback(ML4WebLog.getErrCode("Crypto_API_pkcs7"), {
                    errCode: 888,
    ...
    dsencrypt: function(b) {
        var a = magicjs.cipher.create(true, "SEED-CBC", magicjs.base64.decode("wqpRRHAjpW48fSp6Sd2g7Q=="));
        ...
        init: function() {
            try {
                if (arguments.length == 0) {
                    $.cachedScript("js/util/ML4Web_ErrorCode.js").done(function(script, status) {
                        eval(script);
                        errorCodeObject = ErrorCodeObject
                    })
                } else {
                    var ErrorCodeJS = arguments[0] + "/ML4Web_ErrorCode.js";
                    $.cachedScript(ErrorCodeJS).done(function(script, status) {
                        eval(script);
                        errorCodeObject = ErrorCodeObject
                    })
                }
