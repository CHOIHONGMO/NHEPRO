...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
	<script>
		...
		var grid;
		var baseUrl = "/nhepro/CCTR/CCTR0052";
		var userFlag = false;
		...
		function doGuarApp() {
			...
			EVF.confirm("신청하시겠습니까?", function() {
				...
				store.load(baseUrl + "/cctr0051_doSave.so", function () {
					return EVF.alert("${CCTR0052_0001}"); // 성공적으로 신청 되었습니다.
				});
			});
		}
		...
		function doSearch() {
			...
			store.setGrid([grid]);
			store.load(baseUrl + "/cctr0052_doSearch.so", function () {
	...
	<e:window id="CCTR0052" initData="${initData}" onReady="init" title="${fullScreenName }" breadCrumbs="${breadCrumb }">
