...
<e:ui>
	...
	var baseUrl = "/nhepro/CCPI/CCPI1100";
	...
	function init() {
		...
		gridV.setProperty("singleSelect", ${singleSelect});
  		...
  		gridV.setColGroup([
			...
			,{
                "groupName": '휴게시작시간',
                "columns": ['REST_TIME_START_HOUR', 'REST_TIME_START_MIN']
            }
            ,{
                "groupName": '휴게종료시간',
                "columns": ['REST_TIME_END_HOUR','REST_TIME_END_MIN']
            }
            ,{
                "groupName": '휴게시간',
        	...
        	,{
                "groupName": '교통보조',
                "columns": ['COMU_AS_PAY', 'COMU_AS_RMKS']
            }
            ,{
                "groupName": '업무수당',
		...
		gridV.cellChangeEvent(function (rowIdx, colIdx, irow, icol, value, oldValue) {
			...
			var trafficPay = gridV.getCellValue(rowIdx, "COMU_AS_PAY");
			var etcPay  = gridV.getCellValue(rowIdx, "ETC_PAY");

			
            if(colIdx == "BASE_PAY") {
            	gridV.setCellValue(rowIdx, "WORKER_PAY_AMT", Number(value) + Number(foodPay) + Number(extendPay) 
            			+ Number(workPay) + Number(trafficPay)+ Number(etcPay));
            }
            
            if(colIdx == "FOOD_PAY") {
            	gridV.setCellValue(rowIdx, "WORKER_PAY_AMT", Number(value) + Number(basePay) + Number(extendPay) 
            			+ Number(workPay) + Number(trafficPay)+ Number(etcPay));
            }           
            
            if(colIdx == "EXTEND_PAY") {
            	gridV.setCellValue(rowIdx, "WORKER_PAY_AMT", Number(value) + Number(basePay) + Number(foodPay)
            		    + Number(workPay) + Number(trafficPay)+ Number(etcPay));
            }
            
            if(colIdx == "WORK_PAY") {
            	gridV.setCellValue(rowIdx, "WORKER_PAY_AMT", Number(value) + Number(basePay) + Number(foodPay)
            			+ Number(extendPay) + Number(trafficPay)+ Number(etcPay));
            }
            
            
            if(colIdx == "COMU_AS_PAY") {
            	gridV.setCellValue(rowIdx, "WORKER_PAY_AMT", Number(value) + Number(basePay) + Number(foodPay)
            			+ Number(extendPay) + Number(workPay)+ Number(etcPay));
            }
            
            if(colIdx == "ETC_PAY") {
            	gridV.setCellValue(rowIdx, "WORKER_PAY_AMT", Number(value) + Number(basePay) + Number(foodPay)
            			+ Number(extendPay) + Number(workPay) + Number(trafficPay));
            }
	...
	function doSearchEmployee() {
		...
		store.setGrid([gridV]);
		store.load(baseUrl + "/ccpi1100_getSavedEmpListForBundleContract.so", function () {
			gridV.setCellValue(rowIdx, 'EMP_ATT_FILE_CNT', fileCnt);
		}, false);
	}
	...
	function doSetDefaultForm(){
		...
		//-----전자근로계약시스템 서명 대기 상태일 떄 로직 처리 하기---------  
	...
	function doSave() {
		...
		EVF.confirm("${msg.M0021 }", function () {
			...
			store.doFileUpload(function () {
				store.load(baseUrl + "/ccpi1100_doSave.so", function () {
	...
	function checkFormValidation() {
		...
		for (var i = 0; i < rowIds.length; i++) {
			...
			var dispatchStartDate = gridV.getCellValue(rowIds[i], 'DISPATCH_START_DATE');
			var dispatchEndDate = gridV.getCellValue(rowIds[i], 'DISPATCH_END_DATE');
	...
	function doCopy() {
		...
		//행 복사 적용항목 : 근로시작시간, 근로종료시간, 휴게시작시간, 휴게종료시간, 휴게시간, 각 수당 별 지급조건
		EVF.confirm('${CCPI1100_010}', function() {

				for(var i in rowIds) {
					gridV.setCellValue(i, "WORK_TIME_START_HOUR", gridV.getCellValue(selectedRow, 'WORK_TIME_START_HOUR'));
					gridV.setCellValue(i, "WORK_TIME_START_MIN", gridV.getCellValue(selectedRow, 'WORK_TIME_START_MIN'));
					gridV.setCellValue(i, "WORK_TIME_END_HOUR", gridV.getCellValue(selectedRow, 'WORK_TIME_END_HOUR'));
					gridV.setCellValue(i, "WORK_TIME_END_MIN", gridV.getCellValue(selectedRow, 'WORK_TIME_END_MIN'));
					gridV.setCellValue(i, "REST_TIME_START_HOUR", gridV.getCellValue(selectedRow, 'REST_TIME_START_HOUR'));
					gridV.setCellValue(i, "REST_TIME_START_MIN", gridV.getCellValue(selectedRow, 'REST_TIME_START_MIN'));
					gridV.setCellValue(i, "REST_TIME_END_HOUR", gridV.getCellValue(selectedRow, 'REST_TIME_END_HOUR'));
					gridV.setCellValue(i, "REST_TIME_END_MIN", gridV.getCellValue(selectedRow, 'REST_TIME_END_MIN'));
					...
					gridV.setCellValue(i, "COMU_AS_RMKS", gridV.getCellValue(selectedRow, 'COMU_AS_RMKS'));
	...
	function doReqSign() {
		...
		store.doFileUpload(function () {
			store.load(baseUrl + "/ccpi1100_doReqSign.so", function () {
				// 저장 완료 후 ozd, pdf 저장
				var bundleNum = this.getParameter('BUNDLE_NUM');
				var contType  = this.getParameter("CONT_TYPE");
				if (opener) {
	...
	function doDelete() {
		EVF.confirm("${msg.M0013}", function () {
			...
			store.load(baseUrl + "/ccpi1100_doDeleteBundleContract.so", function () {
				EVF.alert("${msg.M0001}");
				if (opener) {
					opener.doSearch();
					doClose();
	...
	function doSavePdf() {
		...
		EVF.confirm("${msg.M0021 }", function () {
			...
			store.doFileUpload(function () {
				store.load(baseUrl + "/ccpi1100_doSavePdf.so", function () {
  			...
  			if(jsonData[idx].SUBCUST_NAME != ''  && jsonData[idx].SUBCUST_NAME  != null){
       		...
       		var addParam = [{
        			...
        			"BIRTH_DATE" : jsonData[idx].BIRTH_DATE,
 					...
 					"REST_TIME_START_HOUR" : restStartHour,
 					"REST_TIME_START_MIN" : startMin,
 					"REST_TIME_END_HOUR" : restEndHour,
 					"REST_TIME_END_MIN" : endMin,
        			...
        			"G_PLACE" : gplace,
 					...
 					"COMU_AS_PAY" : jsonData[idx].COMU_AS_PAY,
 					"WORK_PAY" : jsonData[idx].WORK_PAY,
 					"EXTEND_PAY" : jsonData[idx].EXTEND_PAY,
 					//"WORKER_PAY_AMT" : (parseInt(jsonData[idx].BASE_PAY) + parseInt(jsonData[idx].EXTEND_PAY) + parseInt(jsonData[idx].FOOD_PAY) 
 					//+ parseInt(jsonData[idx].COMU_AS_PAY) + parseInt(jsonData[idx].WORK_PAY) + parseInt(jsonData[idx].ETC_PAY)),
 					"WORK_DAY" : 5,
 					"DISPATCH_TASK" : jsonData[idx].DISPATCH_TASK,
 					"ETC_PAY" : jsonData[idx].ETC_PAY3,
 					"ETC_PAY_RMKS" : jsonData[idx].ETC_PAY3_RMKS,
    ...
    function workerPayAmtData(){
    	...
    	for(var i in rowIds){
			...
			var trafficPay = gridV.getCellValue(i, "COMU_AS_PAY");
			var etcPay  = gridV.getCellValue(i, "ETC_PAY");
		
      
        gridV.setCellValue(i, "WORKER_PAY_AMT", Number(basePay) + Number(extendPay) + Number(foodPay) + Number(workPay) 
        		+ Number(trafficPay)+ Number(etcPay));
	...
	function doSearchForm() {
		...
		//store.load("/nhepro/CCTR/CCTA0030/ccta0030_doSearchMainForm.so", function () {
		  store.load(baseUrl + "/ccpi1100_doSearchMainForm.so", function () {
	...
	function doSearchSubForms() {
		...
		if (gridM.isExistsSelRow()) {
			...
			store.setGrid([gridA]);
			store.load(baseUrl + "/ccpi1100_doSearchAdditionalForm.so", function () {	
		...
		/* 		if(!gridM.isExistsSelRow()) {
				...
				if( CONT_NUM != "" && CONT_CNT != "" ) {
					if ("${form.PROGRESS_CD}" == "4300" && GRD_PROGRESSCD =="4300" ) {//임시저장 , 결재중, 근로자서명대기				
						...
						store.load(baseUrl + '/ccpi1100_doSelectPdfJsonData.so', function() {
...
/* 		var eformInputValue = "";
	...
	<e:window id="CCPI1100" onReady="init" initData="${initData}" title="${fullScreenName }" breadCrumbs="${breadCrumb }" margin="0 4px">
...
<%--
	<e:title title="첨부파일"/>
	<e:searchPanel id="form2" title="${form_CAPTION_N }" columnCount="1" labelWidth="125" onEnter="doSearch" useTitleBar="false">
		<e:row>
