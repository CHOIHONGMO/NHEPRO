...
<e:ui>
	...
	function init() {
		...
		gridV.setColGroup([
			...
			{
                "groupName": '근로시작시간',
                "columns": ['WORK_TIME_START_HOUR', 'WORK_TIME_START_MIN']
            }
            ,{
                "groupName": '근로종료시간',
                "columns": ['WORK_TIME_END_HOUR','WORK_TIME_END_MIN']
            }
            ,{
                "groupName": '휴게시간',
                "columns": ['REST_TIME_HOUR', 'REST_TIME_MIN']
            }
        	,{
                "groupName": '기본급',
        	...
        	,{
                "groupName": '자격수당',
                "columns": ['LICENSE_PAY', 'LICENSE_PAY_RMKS']
            }
        	,{
                "groupName": '직책수당',
		...
		gridM.cellClickEvent(function (rowId, colId, value) {
			...
			if (colId === "multiSelect" || colId === "FORM_NM" || colId === "CONTRACT_FORM_TYPE" || colId === "AUTO_RENEW_FLAG") {
				
				if( (changeFlag == "0" || changeFlag =="1")  && (rowId =="2" || rowId == "3")){
					gridV.delAllRow();
				}else if( (changeFlag == "2" || changeFlag =="3")  && (rowId =="0" || rowId == "1")){
					gridV.delAllRow();
				}
			}
		...
		//월지급액 계산
		gridV.cellChangeEvent(function (rowIdx, colIdx, irow, icol, value, oldValue) {
			
			var basePay   = gridV.getCellValue(rowIdx, "BASE_PAY");
			var licensePay = gridV.getCellValue(rowIdx, "LICENSE_PAY");
			var jobPay    = gridV.getCellValue(rowIdx, "JOB_PAY");
			var extendPay = gridV.getCellValue(rowIdx, "EXTEND_PAY");
			var foodPay   = gridV.getCellValue(rowIdx, "FOOD_PAY");
			var workPay   = gridV.getCellValue(rowIdx, "WORK_PAY");
			var nightPay  = gridV.getCellValue(rowIdx, "NIGHT_PAY");
			var etcPay  = gridV.getCellValue(rowIdx, "ETC_PAY");
			var etcPay2  = gridV.getCellValue(rowIdx, "ETC_PAY2");
			
            if(colIdx == "BASE_PAY") {
            	gridV.setCellValue(rowIdx, "WORKER_PAY_AMT", Number(value) + Number(foodPay) + Number(jobPay) + Number(extendPay) 
            			+ Number(workPay) + Number(nightPay) + Number(licensePay) + Number(etcPay)+ Number(etcPay2));
            }
            
            if(colIdx == "FOOD_PAY") {
            	gridV.setCellValue(rowIdx, "WORKER_PAY_AMT", Number(value) + Number(basePay) + Number(jobPay) + Number(extendPay) 
            			+ Number(workPay) + Number(nightPay) + Number(licensePay)+ Number(etcPay)+ Number(etcPay2));
            }
            
            if(colIdx == "JOB_PAY") {
            	gridV.setCellValue(rowIdx, "WORKER_PAY_AMT", Number(value) + Number(basePay) + Number(foodPay) + Number(extendPay) 
            			+ Number(workPay) + Number(nightPay) + Number(licensePay)+ Number(etcPay)+ Number(etcPay2));
            }
            
            if(colIdx == "EXTEND_PAY") {
            	gridV.setCellValue(rowIdx, "WORKER_PAY_AMT", Number(value) + Number(basePay) + Number(foodPay) + Number(jobPay)
            		    + Number(workPay) + Number(nightPay) + Number(licensePay)+ Number(etcPay)+ Number(etcPay2));
            }
            
            if(colIdx == "WORK_PAY") {
            	gridV.setCellValue(rowIdx, "WORKER_PAY_AMT", Number(value) + Number(basePay) + Number(foodPay) + Number(jobPay)
            			+ Number(extendPay) + Number(nightPay) + Number(licensePay)+ Number(etcPay)+ Number(etcPay2));
            }
            
            if(colIdx == "NIGHT_PAY") {
            	gridV.setCellValue(rowIdx, "WORKER_PAY_AMT", Number(value) + Number(basePay) + Number(foodPay) + Number(jobPay)
            			+ Number(extendPay) + Number(workPay) + Number(licensePay)+ Number(etcPay)+ Number(etcPay2));
            }
            
            if(colIdx == "LICENSE_PAY") {
            	gridV.setCellValue(rowIdx, "WORKER_PAY_AMT", Number(value) + Number(basePay) + Number(foodPay) + Number(jobPay)
            			+ Number(extendPay) + Number(workPay) + Number(nightPay)+ Number(etcPay)+ Number(etcPay2));
            }
            if(colIdx == "ETC_PAY") {
            	gridV.setCellValue(rowIdx, "WORKER_PAY_AMT", Number(value) + Number(basePay) + Number(foodPay) + Number(jobPay)
            			+ Number(extendPay) + Number(workPay) + Number(nightPay)+Number(licensePay)+ Number(etcPay2));
            }
            if(colIdx == "ETC_PAY2") {
            	gridV.setCellValue(rowIdx, "WORKER_PAY_AMT", Number(value) + Number(basePay) + Number(foodPay) + Number(jobPay)
            			+ Number(extendPay) + Number(workPay) + Number(nightPay)+Number(licensePay)+ Number(etcPay));
            }
	    ...
	    //gridV.getCellValue(rowIdx, "EMP_ATT_FILE_NUM");
	    // 개인근로자 조회
		if (EVF.V("BUNDLE_NUM") != "") {
	...
	function doSearchEmployee() {
		...
		store.setGrid([gridV]);
		store.load(baseUrl + "/ccpi1000_getSavedEmpListForBundleContract.so", function () {
	...
	function doSave() {
		...
		EVF.confirm("${msg.M0021 }", function () {
			...
			store.doFileUpload(function () {
				store.load(baseUrl + "/ccpi1000_doSave.so", function () {
	...
	function checkFormValidation() {
		...
		for (var i = 0; i < rowIds.length; i++) {
			...
			var specialCondition = gridV.getCellValue(rowIds[i], 'SPECIAL_CONDITION');
	...
	function doCopy() {
		...
		EVF.confirm('${CCPI1000_009}', function() {

				for(var i in rowIds) {
					
					gridV.setCellValue(i, "WORK_CONDITION", gridV.getCellValue(selectedRow, 'WORK_CONDITION'));
					gridV.setCellValue(i, "REST_CONDITION", gridV.getCellValue(selectedRow, 'REST_CONDITION'));
					
					gridV.setCellValue(i, "BASE_PAY_RMKS", gridV.getCellValue(selectedRow, 'BASE_PAY_RMKS'));
					gridV.setCellValue(i, "LICENSE_PAY_RMKS", gridV.getCellValue(selectedRow, 'LICENSE_PAY_RMKS'));
					gridV.setCellValue(i, "JOB_PAY_RMKS", gridV.getCellValue(selectedRow, 'JOB_PAY_RMKS'));
					gridV.setCellValue(i, "EXTEND_PAY_RMKS", gridV.getCellValue(selectedRow, 'EXTEND_PAY_RMKS'));
					gridV.setCellValue(i, "FOOD_PAY_RMKS", gridV.getCellValue(selectedRow, 'FOOD_PAY_RMKS'));
					gridV.setCellValue(i, "WORK_PAY_RMKS", gridV.getCellValue(selectedRow, 'WORK_PAY_RMKS'));
					gridV.setCellValue(i, "NIGHT_PAY_RMKS", gridV.getCellValue(selectedRow, 'NIGHT_PAY_RMKS'));
	...
	function doReqSign() {
		...
		store.doFileUpload(function () {
			store.load(baseUrl + "/ccpi1000_doReqSign.so", function () {
				// 저장 완료 후 ozd, pdf 저장
				var bundleNum = this.getParameter('BUNDLE_NUM');
				var contType  = this.getParameter("CONT_TYPE");
				if (opener) {
	...
	function doDelete() {
		EVF.confirm("${msg.M0013}", function () {
			...
			store.load(baseUrl + "/ccpi1000_doDeleteBundleContract.so", function () {
				EVF.alert("${msg.M0001}");
				if (opener) {
					opener.doSearch();
					doClose();
	...
	function doSavePdf() {
		...
		EVF.confirm("${msg.M0021 }", function () {
			...
			store.doFileUpload(function () {
				store.load(baseUrl + "/ccpi1000_doSavePdf.so", function () {
	...
	function doEmpAdd() {
		EVF.confirm("기본서식을 선택하셨습니까?"+"<br>"+"(※ 근로자 추가 후 서식변경 시 그리드가 초기화 될 수 있습니다.)", function () {
    	var param = {
    ...
    function _setEmployee(jsonData) {
    	...
    	var restHour = '01';
    	var restMin = '00';
    	var etcPay, etcPayRmk;
    	var etcPay2,etcPay2Rmk;
    	var jobPay, nightPay;
   		 ...
   		 for( idx in jsonData ){
   			...
   			//기타수당 및 내역 (도급, 촉탁에 따른 조건 처리)
   			if( (gridM.getSelRowValue()[0].FORM_NM).substring(0,2) == "도급"){
   				etcPay = jsonData[idx].ETC_PAY;
   				etcPayRmk = jsonData[idx].ETC_PAY_RMKS;
   				etcPay2 = "";
   				etcPay2Rmk = "";
   				jobPay = jsonData[idx].JOB_PAY;
   				nightPay = jsonData[idx].NIGHT_PAY;
   				gridV.setColReadOnly("ETC_PAY2", true);
   				gridV.setColReadOnly("ETC_PAY2_RMKS", true);
   				gridV.setColReadOnly("ETC_PAY", false);
   				gridV.setColReadOnly("ETC_PAY_RMKS", false);
   			}else{ //촉탁직인 경우
   				etcPay2 = jsonData[idx].ETC_PAY2;
   				etcPay2Rmk = jsonData[idx].ETC_PAY2_RMKS;
   				etcPay = "";
   				etcPayRmk = "";
   				jobPay = "";
   				nightPay = "";
   				gridV.setColReadOnly("ETC_PAY", true);
   				gridV.setColReadOnly("ETC_PAY_RMKS", true);
   				gridV.setColReadOnly("ETC_PAY2", false);
   				gridV.setColReadOnly("ETC_PAY2_RMKS", false);
   			}
       		...
       		var addParam = [{
        			...
        			"BIRTH_DATE" : jsonData[idx].BIRTH_DATE,
 					"WORK_TIME_START_HOUR" : startHour,
 					"WORK_TIME_START_MIN" : startMin,
 					"WORK_TIME_END_HOUR" : endHour,
 					"WORK_TIME_END_MIN" : endMin,
 					...
 					"BASE_PAY" : jsonData[idx].BASE_PAY,
 					"LICENSE_PAY" : jsonData[idx].LICENSE_PAY,
 					"JOB_PAY" : jobPay,
 					"EXTEND_PAY" : jsonData[idx].EXTEND_PAY,
 					"FOOD_PAY" : jsonData[idx].FOOD_PAY,
 					"WORK_PAY" : jsonData[idx].WORK_PAY,
 					"NIGHT_PAY" : nightPay,
 					//"WORKER_PAY_AMT" : parseInt(jsonData[idx].BASE_PAY) + parseInt(jsonData[idx].LICENSE_PAY) + parseInt(jsonData[idx].JOB_PAY)
 					//+ parseInt(jsonData[idx].EXTEND_PAY) + parseInt(jsonData[idx].FOOD_PAY) + parseInt(jsonData[idx].NIGHT_PAY)+parseInt(jsonData[idx].ETC_PAY),
 					"POSITION" : jsonData[idx].POSITION, 
 					"ETC_PAY" : etcPay,
 					"ETC_PAY_RMKS" : etcPayRmk,
 					"ETC_PAY2" : etcPay2,
 					"ETC_PAY2_RMKS" : etcPay2Rmk,
 					"CUST_NAME" : jsonData[idx].CUST_NAME,
 					"SUBCUST_NAME" : jsonData[idx].SUBCUST_NAME,
 					"G_ADDR1" : jsonData[idx].G_ADDR1+" "+jsonData[idx].G_ADDR2
    			}];
    ...
    function workerPayAmtData(){
    	var rowIds = gridV.getAllRowId();
    	
    	for(var i in rowIds){
    	var basePay   = gridV.getCellValue(i, "BASE_PAY");
		var licensePay = gridV.getCellValue(i, "LICENSE_PAY");
		var jobPay    = gridV.getCellValue(i, "JOB_PAY");
		var extendPay = gridV.getCellValue(i, "EXTEND_PAY");
		var foodPay   = gridV.getCellValue(i, "FOOD_PAY");
		var workPay   = gridV.getCellValue(i, "WORK_PAY");
		var nightPay  = gridV.getCellValue(i, "NIGHT_PAY");
		var etcPay  = gridV.getCellValue(i, "ETC_PAY");
		var etcPay2  = gridV.getCellValue(i, "ETC_PAY2");
		
        gridV.setCellValue(i, "WORKER_PAY_AMT", Number(basePay) + Number(licensePay) + Number(jobPay) + Number(foodPay) 
        		+ Number(extendPay)+ Number(workPay) + Number(nightPay)  + Number(etcPay) + Number(etcPay2));
	...
	function doSearchForm() {
		...
		//store.load("/nhepro/CCTR/CCTA0030/ccta0030_doSearchMainForm.so", function () {
		  store.load(baseUrl + "/ccpi1000_doSearchMainForm.so", function () {
	...
	function doSearchSubForms() {
		...
		if (gridM.isExistsSelRow()) {
			...
			store.setGrid([gridA]);
			store.load(baseUrl + "/ccpi1000_doSearchAdditionalForm.so", function () {	
...
/* 		if(!gridM.isExistsSelRow()) {
		...
		if( CONT_NUM != "" && CONT_CNT != "" ) {
			if ("${form.PROGRESS_CD}" == "4300" && GRD_PROGRESSCD =="4300" ) {//임시저장 , 결재중, 근로자서명대기				
				//pdfAttFileNum
				...
				store.load(baseUrl + '/ccpi1000_doSelectPdfJsonData.so', function() {
	...
	function ozdViewAndSave(CONT_NUM, CONT_CNT, EMP_ATT_FILE_NUM) {
		...
		// 서브 폼 파일명을 가져온다.
		var subFormFileNm = "";
		var subFormFileNmTest = "";
		for(var i in gridA.getSelRowValue()) {
			var value = gridA.getSelRowValue()[i];
			subFormFileNm += value.FORM_FILE_NM + ",";
			subFormFileNmTest += value.REL_FORM_NM+ ",";
		}
		console.log("부서식명 : =======>"+subFormFileNm.substring(0, subFormFileNm.length - 1));
		console.log("부서식명2 : =======>"+subFormFileNmTest.substring(0, subFormFileNmTest.length - 1));
		...
		var param = {
				...
				DISPLAY_NM : subFormFileNmTest.substring(0, subFormFileNmTest.length - 1),
				odiName: "CONTRACT",
	...
	<e:window id="CCPI1000" onReady="init" initData="${initData}" title="${fullScreenName }" breadCrumbs="${breadCrumb }" margin="0 4px">
