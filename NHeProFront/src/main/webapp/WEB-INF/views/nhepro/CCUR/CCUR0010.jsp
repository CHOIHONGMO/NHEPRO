...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    ...
    <script type="text/javascript">
		...
		function doSearchTs(){
			...
			store.load(baseUrl + 'ccur0010_doSearchTs.so', function() {
				if(gridTS.getRowCount() > 0){

					gridTS.checkAll(true);
					<%-- 수정시 첨부파일 위에서 3개 필수에서 제외처리, 첨부하지 않아도 수정되게 처리 --%>
					var rowIds = gridTS.getAllRowId();
					for(var i in rowIds) {
						//gridTS.setCellValue(rowIds[i], 'REQUIRED_FLAG', '0');
						<%--
        ...
        function doSave() {
			...
			for(var i in rowIds) {
				...
				if(gridTS.getCellValue(rowIds[i], 'REQUIRED_FLAG') == "1" && !EVF.isEmpty(gridTS.getCellValue(rowIds[i], 'VALID_PERIOD'))) {
					if(EVF.isEmpty(gridTS.getCellValue(rowIds[i], 'VALID_START_DATE')) || EVF.isEmpty(gridTS.getCellValue(rowIds[i], 'VALID_END_DATE'))) {
						return EVF.alert(gridTS.getCellValue(rowIds[i], 'TMPL_FILE_NM') + "의 " + "${CCUR0010_026}");
					}
				}*/
				...
				if(gridTS.getCellValue(rowIds[i], 'VALID_START_DATE') > gridTS.getCellValue(rowIds[i], 'VALID_END_DATE')) {
					return EVF.alert(gridTS.getCellValue(rowIds[i], 'TMPL_FILE_NM') + "의 " + "${CCUR0010_027}");
				}
			}
            ...
            EVF.confirm(btnMsg, function () {
				store.doFileUpload(function() {
					...
					store.load(baseUrl + 'ccur0010_doSave.so', function () {
						...
						EVF.alert(this.getResponseMessage(), function() {
							if (!EVF.isEmpty(custCd)) {
								...
								window.location.href = '/nhepro/CCUR/CCUR0010/view.so?' + $.param(param);
							}
						});
					});
				});
		...
		function checkIrsNum() {
			...
			if( !isIrsNum(EVF.V("IRS_NUM")) ) {
				...
				var store = new EVF.Store();
				store.load(baseUrl + 'ccur0010_checkIrsNum.so', function() {
					if(this.getParameter("POSSIBLE_FLAG") == "N") {
						return EVF.alert("${msg.M0127}");
					}
				}, false);
			}
		}
	...
	<e:window id="CCUR0010" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
		...
		<e:searchPanel id="form1" title="" labelWidth="${labelWidth}" width="100%" columnCount="3" useTitleBar="false" collapsedIcon="true">
			<e:row>
			 ...
			 <e:row>
				...
				<e:field>
					<e:select id="COMPANY_TYPE" name="COMPANY_TYPE" value="${formData.COMPANY_TYPE }" options="${companyTypeOptions}" width="${form_COMPANY_TYPE_W}" disabled="${form_COMPANY_TYPE_D}" readOnly="${form_COMPANY_TYPE_RO}" required="${form_COMPANY_TYPE_R}" placeHolder=""  maskType="${form_COMPANY_TYPE_MT}"/>
				</e:field>
			...
			<e:row>
				...
				<e:field>
					<e:inputText id="IRS_NUM" name="IRS_NUM" placeHolder="${CCUR0010_INPUT_T1 }" value="${formData.IRS_NUM }" width="${form_IRS_NUM_W}" maxLength="${form_IRS_NUM_M}" disabled="${form_IRS_NUM_D}" readOnly="${form_IRS_NUM_RO}" required="${form_IRS_NUM_R}" onChange="checkIrsNum" maskType="${form_IRS_NUM_MT}" />
				</e:field>
				<e:label for="COMPANY_REG_NUM" title="${form_COMPANY_REG_NUM_N}" />
				<e:field>
					<e:inputText id="COMPANY_REG_NUM" name="COMPANY_REG_NUM" placeHolder="${CCUR0010_INPUT_T1 }" value="${formData.COMPANY_REG_NUM }" width="${form_COMPANY_REG_NUM_W}" maxLength="${form_COMPANY_REG_NUM_M}" disabled="${form_COMPANY_REG_NUM_D}" readOnly="${form_COMPANY_REG_NUM_RO}" required="${form_COMPANY_REG_NUM_R}" onChange="checkCompanyRegNum"  maskType="${form_COMPANY_REG_NUM_MT}" />
				</e:field>
			...
			<e:row>
				...
				<e:field>
					<e:inputText id="TEL_NUM" name="TEL_NUM" placeHolder="${CCUR0010_INPUT_T2 }" value="${formData.TEL_NUM }" width="${form_TEL_NUM_W}" maxLength="${form_TEL_NUM_M}" disabled="${form_TEL_NUM_D}" readOnly="${form_TEL_NUM_RO}" required="${form_TEL_NUM_R}" data="TEL_NUM" onChange="checkTelNo" maskType="${form_TEL_NUM_MT}" />
				</e:field>
				<e:label for="FAX_NUM" title="${form_FAX_NUM_N}" />
				<e:field>
					<e:inputText id="FAX_NUM" name="FAX_NUM" placeHolder="${CCUR0010_INPUT_T2 }" value="${formData.FAX_NUM }" width="${form_FAX_NUM_W}" maxLength="${form_FAX_NUM_M}" disabled="${form_FAX_NUM_D}" readOnly="${form_FAX_NUM_RO}" required="${form_FAX_NUM_R}" data="FAX_NUM" onChange="checkTelNo" maskType="${form_FAX_NUM_MT}" />
				</e:field>
				<e:label for="EMAIL" title="${form_EMAIL_N}"/>
				<e:field>
					<e:inputText id="EMAIL" name="EMAIL" placeHolder="${CCUR0010_INPUT_T3 }" value="${formData.EMAIL }" width="${form_EMAIL_W}" maxLength="${form_EMAIL_M}" disabled="${form_EMAIL_D}" readOnly="${form_EMAIL_RO}" required="${form_EMAIL_R}" style="${imeMode}" maskType="${form_EMAIL_MT}" />
				</e:field>
			</e:row>
			<e:row>
		...
		<e:panel id="leftP3" height="25px" width="100%">
			...
			<span style="position: relative; left: 80px; top: -23px; font-size: 11px; color: red;">${CCUR0010_TEXT5}</span>
			<span style="position: relative; top: -18px; font-size: 11px; float: right; font-weight: bold; right: 2px;">${CCUR0010_SUB_T}</span>
		</e:panel>
