...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
	<script>
		...
		var grid;
		var baseUrl = "/nhepro/CCTR/CCTR0054";
		...
		function init() {
			...
			grid._gvo.setFooter({
				'mergeCells' : ["PR_BUYER_DEPT_NM", "DI_GUAR_TYPE"]
			});
				...
				switch (celName) {
					case "DI_ATT_FILE_CNT":
						if(value != 0) {
							param = {
								bizType: 'EC',
								attFileNum: grid.getCellValue(rowIdx, 'DI_ATT_FILE_NUM'),
								detailView: true
							};
		...
		function doSearch() {
			...
			store.setGrid([grid]);
			store.load(baseUrl + "/cctr0054_doSearch.so", function () {
				...
				grid.setRowFooter("DI_GUAR_AMT", footer1);
			});
		}

	</script>

	<e:window id="CCTR0054" initData="${initData}" onReady="init" title="${fullScreenName }" breadCrumbs="${breadCrumb }">
