...
import com.st_ones.batch.nhebatch.service.BNH0013_Service;
...
@Service
public class BNH0013 extends EverJob {
    @Autowired private BNH0001_Service bnh0001_service;
    @Autowired private BNH0013_Service bnh0013_service;
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (super.isAllowedToRunBatch(context)) {
            ...
            bnh0013_service = SpringContextUtil.getBean(BNH0013_Service.class);

            String msg = "";
            int totalCount = 0;

            try {
                msg = bnh0013_service.doExecService(new HashMap<String, String>());
                jobRlt = "S";
            } catch (Exception e) {
            	...
            	if( telNums != null ) {
	            	for (String telNum : telNums) {
	                    try {
	    	                ...
	    	                smsMap.put("CONTENTS", "[전자구매Batch] 첨부파일 과금(BNH0013) I/F시 오류가 발생");
                ...
                try {
                    ...
                    logData.put("JOB_ID", "BNH0013");
                    logData.put("JOB_NM", "첨부파일 과금");
                    logData.put("JOB_KEY", "");
