...
public class TestLIVINF {
	...
	public static String composeXML(String templatePath, String docCode)
	{
		...
		String	head_mesg_type  ="LIVINF";			/*	[필수]	 문서코드	*/
		String	head_mesg_name  ="생활안정통보서";	/*	[필수]	 문서명	*/
		...
		String	head_titl_name  ="생활안정통보서";	/*	[필수]	 문서개요	*/
		String  head_orga_code  ="ABC";             /*  [필수]   연계코드   */
		/* 보험계약정보 */								
		String	bond_kind_code  ="003";				/*	[필수]	" 보험종목구분(002:계약,003:하자,004:선금,006:지급)"	*/
		String	bond_begn_date  ="20060101";		/*	[필수]	 보험개시일자	*/
		String	bond_fnsh_date  ="20062131";		/*	[필수]	 보험종료일자	*/
		String	bond_curc_code  ="WON";				/*	[필수]	" 보험가입금액/통화코드(WON, USD …)"	*/
		String	bond_penl_amnt  ="20000000";		/*	[필수]	 보험가입금액/보험가입금액	*/
		String	bond_oper_code  ="SELECT";				/*	[선택]	" 조회등록 업무구분(SELECT,INSERT)"	*/
		String	bond_appl_code  ="20";				/*	[필수]	" 신규배서 업무구분(10:신규,20:연장,60:증액,62:연장증액,70:감액,90:기타)"	*/
		/* 주요계약정보 */								
		...
		String	cont_begn_date  ="20060507";		/*  [필수]   계약시작일자 */
		String	cont_fnsh_date  ="20060507";		/*  [필수]   계약종료일자 */
		String	cont_term_text  ="";				/*  [선택]   계약기간 */
		String	cont_pric_rate  ="20";				/*  [필수]   보증금율 */
		String	hist_bond_numb  ="12000";			/*  [선택]   차환대환 증권번호 */
		/* 대출상환정보 */
		String	capt_agre_code  ="ez";				/*  [필수]    협약코드		   */      
		String	capt_repy_unit  ="E";				/*  [필수]    상환단위		     */    
		String	capt_repy_date  ="20060507";		/*  [필수]    초회상환일자		 */	
		String	capt_totl_cnts  ="1";				/*  [필수]    전체상환회수			 */
		String	capt_repy_amnt  ="10000";			/*  [필수]    회차별상환금액	 */		
		String	capt_remd_divs  ="B";				/*  [필수]    잔금지급방법			 */
		String	capt_plus_amnt  ="10000";			/*  [선택]    월별추가 상환금액		 */
		String	capt_addn_date  ="20060507";		/*  [필수]    체증금액 상환일자		 */
		String	capt_intr_rate  ="40";				/*  [필수]    이자율			  */       
		String	capt_pldg_divs  ="Y";				/*  [필수]    질권설정여부		 */	
		String	capt_debt_amnt  ="10000";			/*  [필수]    채무잔액		 */        
		String	capt_exec_begn  ="20060507";		/*  [필수]    이행시작일자		 */	
		String	capt_exec_fnsh  ="20060507";		/*  [필수]    이행종료일자	  */               
		...
		/* 고객개인정보 */
		String	cust_entr_date  ="20060302";		/*	입사(회원가입)일자			*/
		String	cust_occu_code  ="";				/*	업무직종			*/	
		String	cust_psit_code  ="";				/*	직급				*/        
		String	cust_year_come  ="2000";			/*	년간소득			*/	
		String	cust_retr_date  ="20100302";		/*	예상퇴직일자		*/                
		/* 수요자 정보 */								
		...
		try{
			...
			if(		
				...
				/* Begin of 대출상환정보 */	
				&&  dataToXml.setData("capt_agre_code", capt_agre_code)
				&&  dataToXml.setData("capt_repy_unit", capt_repy_unit)
				&&  dataToXml.setData("capt_repy_date", capt_repy_date)
				&&  dataToXml.setData("capt_totl_cnts", capt_totl_cnts)
				&&  dataToXml.setData("capt_repy_amnt", capt_repy_amnt)
				&&  dataToXml.setData("capt_remd_divs", capt_remd_divs)
				&&  dataToXml.setData("capt_plus_amnt", capt_plus_amnt)
				&&  dataToXml.setData("capt_addn_date", capt_addn_date)
				&&  dataToXml.setData("capt_intr_rate", capt_intr_rate)
				&&  dataToXml.setData("capt_pldg_divs", capt_pldg_divs)
				&&  dataToXml.setData("capt_debt_amnt", capt_debt_amnt)
				&&  dataToXml.setData("capt_exec_begn", capt_exec_begn)
				&&  dataToXml.setData("capt_exec_fnsh", capt_exec_fnsh)
				/* End of 대출상환정보 */
				...
				/* Begin of 고객개인정보 */
				&&  dataToXml.setData("cust_entr_date", cust_entr_date)
				//&&  dataToXml.setData("cust_occu_code", cust_occu_code)
				//&&  dataToXml.setData("cust_psit_code", cust_psit_code)
				&&  dataToXml.setData("cust_year_come", cust_year_come)
				&&  dataToXml.setData("cust_retr_date", cust_retr_date)
				/* End of 고객개인정보 */				
				...
				)
			{
				...
				FileWriteUtil.genFileCreate( "D:/KICA_SGIxLinker/samples/document/sampleLIVINF.xml", xmlDoc);
			}else{
				System.out.println(dataToXml.getErrorMsg());
			}
		}catch(Exception _e){
...
//			String cont_asgn_rate = xmlToData.getData("cont_asgn_rate");
			...
			String cont_term_text = xmlToData.getData("cont_term_text");
			String cont_pric_rate = xmlToData.getData("cont_pric_rate");
			String hist_bond_numb = xmlToData.getData("hist_bond_numb");					
			/* End of 주요계약정보 */	
			/* Begin of 대출상환정보 */	
			String capt_agre_code = xmlToData.getData("capt_agre_code");
			String capt_repy_unit = xmlToData.getData("capt_repy_unit");
			String capt_repy_date = xmlToData.getData("capt_repy_date");
			String capt_totl_cnts = xmlToData.getData("capt_totl_cnts");
			String capt_repy_amnt = xmlToData.getData("capt_repy_amnt");
			String capt_remd_divs = xmlToData.getData("capt_remd_divs");
			String capt_plus_amnt = xmlToData.getData("capt_plus_amnt");
			String capt_addn_date = xmlToData.getData("capt_addn_date");
			String capt_intr_rate = xmlToData.getData("capt_intr_rate");
			String capt_pldg_divs = xmlToData.getData("capt_pldg_divs");
			String capt_debt_amnt = xmlToData.getData("capt_debt_amnt");
			String capt_exec_begn = xmlToData.getData("capt_exec_begn");
			String capt_exec_fnsh = xmlToData.getData("capt_exec_fnsh");
			/* End of 대출상환정보 */     
			/* Begin of 채권자 정보 */	
			...
			/* End of 계약자 정보 */          
			/* Begin of 고객개인정보 */       
			String cust_entr_date = xmlToData.getData("cust_entr_date");
			String cust_occu_code = xmlToData.getData("cust_occu_code");
			String cust_psit_code = xmlToData.getData("cust_psit_code");
			String cust_year_come = xmlToData.getData("cust_year_come");
			String cust_retr_date = xmlToData.getData("cust_retr_date");
			/* End of 고객개인정보 */		
			/* Begin of 수요자 정보 */	  
...
//			System.out.println("cont_asgn_rate=" + cont_asgn_rate);
			...
			System.out.println("cont_term_text=" + cont_term_text);
			System.out.println("cont_pric_rate=" + cont_pric_rate);
			System.out.println("hist_bond_numb=" + hist_bond_numb);
			System.out.println("capt_agre_code=" + capt_agre_code);
			System.out.println("capt_repy_unit=" + capt_repy_unit);
			System.out.println("capt_repy_date=" + capt_repy_date);
			System.out.println("capt_totl_cnts=" + capt_totl_cnts);
			System.out.println("capt_repy_amnt=" + capt_repy_amnt);
			System.out.println("capt_remd_divs=" + capt_remd_divs);
			System.out.println("capt_plus_amnt=" + capt_plus_amnt);
			System.out.println("capt_addn_date=" + capt_addn_date);
			System.out.println("capt_intr_rate=" + capt_intr_rate);
			System.out.println("capt_pldg_divs=" + capt_pldg_divs);
			System.out.println("capt_debt_amnt=" + capt_debt_amnt);
			System.out.println("capt_exec_begn=" + capt_exec_begn);
			System.out.println("capt_exec_fnsh=" + capt_exec_fnsh);
			...
			System.out.println("cust_entr_date=" + cust_entr_date);
			System.out.println("cust_occu_code=" + cust_occu_code);
			System.out.println("cust_psit_code=" + cust_psit_code);
			System.out.println("cust_year_come=" + cust_year_come);
			System.out.println("cust_retr_date=" + cust_retr_date);
