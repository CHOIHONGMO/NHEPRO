...
//		}
	...
	//2022.10.24 개인근로자 파트너스 
	public List<Map<String, Object>> mtua0010_doSearchUserEmp(Map<String, String> param) throws Exception {
		List<Map<String, Object>> userList = mtua0010_Mapper.mtua0010_doSearchUserEmp(param);
		return userList;
	}
	//2022.11.30 개인근로자 파트너스 
	public Map<String, Object> doGetUserTVUR(Map<String, String> formData) throws Exception {
		return mtua0010_Mapper.doGetUserTVUR(formData);
	}
	...
	public String doInitPassword(Map<String, String> generalForm) throws Exception {
		...
		String smsMessage = "[전자구매시스템]임시비밀번호 : "+password+"로 로그인 한 후 비밀번호를 변경하세요";
	...
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void doInitPassword_TVUR(Map<String, String> generalForm) throws Exception {

		// 비밀번호 난수발생 > 업데이트
		String password = RandomStringUtils.randomAlphanumeric(10);
		generalForm.put("PASSWORD", EverEncryption.getEncryptedUserPassword(EverString.nullToEmptyString(password)));
		mtua0010_Mapper.doSaveIssue_TVUR(generalForm);

		// 변경된 비밀번호 문자발송
		Map<String, String> map = new HashMap<String, String>();
		String smsMessage = "[전자근로계약시스템]임시비밀번호 : " + password + "로 로그인 한 후 비밀번호를 변경하세요";

		map.put("RECV_USER_ID",  generalForm.get("USER_ID"));
		map.put("RECV_USER_NM",  generalForm.get("USER_NM"));
		map.put("RECV_TEL_NUM",  generalForm.get("CELL_NUM"));
		map.put("DIRECT_TARGET", generalForm.get("CELL_NUM"));
		map.put("DIRECT_USER_NM", generalForm.get("USER_NM"));
		map.put("SEND_USER_ID",  PropertiesManager.getString("eversrm.userId.default"));
		map.put("SEND_USER_NM",  "SYSTEM");
		map.put("SEND_TEL_NUM",  PropertiesManager.getString("eversrm.system.sms.default.telNo"));
		map.put("CONTENTS",      smsMessage);
	...
	public String doInsertUserInfo_VNGL(Map<String, String> generalForm, List<Map<String, Object>> auGridDatas) throws Exception {
		...
		int transCnt = -1;
		int checkCnt;
		//2022.11.17 파트너스 로직 추가 
		if("T".equals(String.valueOf(generalForm.get("USER_TYPE")))) {
			checkCnt = mtua0010_Mapper.existsUserInformation_TVUR(generalForm);
		}else {
			// USER_ID 중복체크
			checkCnt = mtua0010_Mapper.existsUserInformation_VNGL(generalForm);
		}
		...
		generalForm.put("PASSWORD", EverEncryption.getEncryptedUserPassword(generalForm.get("PASSWORD")));
		//2022.11.17 개인근로자 파트너스 로직 추가 
		if("T".equals(String.valueOf(generalForm.get("USER_TYPE")))) {
			transCnt = mtua0010_Mapper.createUserInformation_TVUR(generalForm);
		}else {
	...
	//2023.01.05 개인근로자 비밀번호 발급 추가
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public String doSaveIssue_TVUR(Map<String, String> generalForm) throws Exception {

		// 입력받은 비밀번호로 업데이트
		generalForm.put("PASSWORD", EverEncryption.getEncryptedUserPassword(generalForm.get("PASSWORD")));

		// 비밀번호저장시 기존비밀번호와 동일하면 리턴
		int checkPW = mtua0010_Mapper.CheckUserInfoPassWordSame_EMP(generalForm);
		if (checkPW > 0) {
			return msg.getMessage("0153");
		}
		mtua0010_Mapper.doSaveIssue_TVUR(generalForm);
