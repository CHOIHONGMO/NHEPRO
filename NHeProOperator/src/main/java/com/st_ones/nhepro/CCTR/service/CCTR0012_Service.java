...
import com.st_ones.nhepro.CCTR.CCTR0012_Mapper;
 ...
 ******************************************************************************
 * </pre>
 * @File Name : CCTR0012_Service.java
 * @date 2020.04.13
 * @version 1.0
 * @see
 */
@Service(value = "CCTR0012_Service")
public class CCTR0012_Service {
    /**
     * The CCTR0012_Mapper.
     */
    @Autowired
    CCTR0012_Mapper cctr0012_Mapper;
    /**
     * The Msg.
     */
	 ...
	 * 경로 :  >  >
	 */
    public List<Map<String,Object>> cctr0012_doSearch(Map<String, String> param) throws Exception{
    	
    	String relatYN = cctr0012_Mapper.ccti0013_getRelatYN(param);
    	param.put("relatYN", relatYN);
        return cctr0012_Mapper.cctr0012_doSearch(param);
    }

    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    public void cctr0012_doCopy(List<Map<String, Object>> gridData) throws Exception {
        ...
        for (Map<String, Object> grid : gridData) {
            ...
            cctr0012_Mapper.cctr0012_doCopyECCF(grid);
            cctr0012_Mapper.cctr0012_doCopyECCR(grid);
        }
    }
    
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    public void cctr0012_doUpdate(List<Map<String, Object>> gridData) throws Exception {
        for (Map<String, Object> grid : gridData) {
            cctr0012_Mapper.cctr0012_doUpdateECCF(grid);
        }
    }
    
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    public void cctr0012_doDelete(List<Map<String, Object>> gridData) throws Exception {
        for (Map<String, Object> grid : gridData) {
            cctr0012_Mapper.cctr0012_doDeleteECCF(grid);
            cctr0012_Mapper.cctr0012_doDeleteECCR(grid);
        }
    }

    public Map<String, String> ccti0013_doSearch(Map<String, String> param) {
        Map<String, String> searchInfo = cctr0012_Mapper.ccti0013_doSearch(param);

        //String formText = cctr0012_Mapper.ccti0013_doSearchFormText(param);
        //searchInfo.put("FORM_TEXT", formText.replaceAll("&#37;", "%").replaceAll("&#39;", "\'"));

        return searchInfo;
    }

    public List<Map<String, Object>> ccti0013_doSearchECCR(Map<String, String> param) {
    	
    
    	String relatYN = cctr0012_Mapper.ccti0013_getRelatYN(param);
    	param.put("relatYN", relatYN);
        return cctr0012_Mapper.ccti0013_doSearchECCR(param);
    }

    // 서식 저장 및 결재상신
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    public String ccti0013_doSave(Map<String, String> formData, List<Map<String, Object>> grid) throws Exception {
        UserInfo userInfo = UserInfoManager.getUserInfo();
        ...
        if (StringUtils.isEmpty(formNum)) {
            ...
            cctr0012_Mapper.ccti0013_doInsertForm(formData);
        } else {
            cctr0012_Mapper.ccti0013_doUpdateForm(formData);
        }
        ...
        for (Map<String, Object> gridData : grid) {
            ...
            if (hasFormNo && isSelected) {
                cctr0012_Mapper.newFormRegistrationDoUpdateGridData(gridData);
            } else if (hasFormNo && !isSelected) {
                cctr0012_Mapper.newFormRegistrationDoDeleteGridData(gridData);
            } else if (!hasFormNo && isSelected) {
                gridData.put("FORM_NUM", formNum);
                int existCount = cctr0012_Mapper.newFormRegistrationGetExistCount(gridData);
                if (existCount == 0) {
                    cctr0012_Mapper.newFormRegistrationDoInsertGridData(gridData);
        ...
        if( "1".equals(formData.get("APP_USE_FLAG")) ){
            ...
            cctr0012_Mapper.doUpdateSignInfo(formData);
    ...
    public String ccti0013_getRelatYN(Map<String, String> param) {
    	        
        return cctr0012_Mapper.ccti0013_getRelatYN(param);
    }    
}
